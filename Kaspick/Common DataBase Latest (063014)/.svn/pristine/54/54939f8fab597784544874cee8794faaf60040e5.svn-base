IF EXISTS (
		...
		FROM sysobjects
		WHERE type = 'P'
			AND NAME = 'USP_EX_GetDonorBenePagingAllInactiveAccounts'
		)
BEGIN
	DROP PROCEDURE USP_EX_GetDonorBenePagingAllInactiveAccounts;

	PRINT 'DROPPED PROCEDURE USP_EX_GetDonorBenePagingAllInactiveAccounts';
END
GO

/******************************************************************************  
** New Name:     USP_EX_GetDonorBenePagingAllInactiveAccounts
** Old Name:     USP_EIS_EX_DONORBENE_PAGING_AllInactiveAccounts_SelProc  
...
*******************************************************************************/
CREATE PROCEDURE [dbo].[USP_EX_GetDonorBenePagingAllInactiveAccounts]
	-- paremeters here  
	@ModuleId INT
...
CREATE TABLE #DONORBENE (
	...
	(
		SELECT Dbn.PARTICIPANT_ID FROM [DBO].[#DONORBENE] Dbn INNER JOIN [DBO].[#DONOR] Dnr  ON Dbn.PARTICIPANT_ID=Dnr.PARTICIPANT_ID
		AND Dbn.ACCOUNT_ID=Dnr.ACCOUNT_ID
	)
...
CREATE TABLE [DBO].[#ALLDONORBENE](
	...
	WHERE
		...
		--		CASE 
		--			WHEN @strFilter <> '' 
		--				AND @strFilter <> '123' AND @SearchColumnName ='Advent_ID'
		--				THEN @strFilter + '%'
		--			WHEN @strFilter = '123'
		--				THEN '[0-9]%'
		...
		--			END
		--		)
		AND DONOR_TYPE IN ('Inactive Donor','Inactive Beneficiary','Inactive Donor/Inactive Beneficiary')
...
BEGIN
	PRINT 'CREATED PROCEDURE USP_EX_GetDonorBenePagingAllInactiveAccounts';
END