IF EXISTS (
		...
		FROM sysobjects
		WHERE type = 'P'
			AND NAME = 'USP_EX_GetAccountPagingAllAccounts'
		)
BEGIN
	DROP PROCEDURE USP_EX_GetAccountPagingAllAccounts;

	PRINT 'DROPPED USP_EX_GetAccountPagingAllAccounts';
END
GO

/************************************************************************
** New Name:     USP_EX_GetAccountPagingAllAccounts  
** Old Name:     USP_EIX_AccountPagingAllAccountsSelProc  
...
**  
** Sample Call  
   EXEC USP_EX_GetAccountPagingAllAccounts  
  @ModuleId  =0,  
 @ClientId  =0,  
 @ProgramID =0,  
 @startrow  =1,  
...
** --------     -------- ------    -------------------------------------- --------  
** 03/13/2014    Sanath  EXCREQ 7.4 Modified 
** 05/22/2014    Sanath  Sp name renamed as per Kaspick naming convention standard  
********************************************************************************/
CREATE PROCEDURE [dbo].[USP_EX_GetAccountPagingAllAccounts] --0,'','', 1,18,'','',0,'','','','adventid' ,''    
	@ModuleId INT
...
AND AccMstr.AllianceNumber = (
	...
	AND AccMstr.ManagerCode = (
		CASE 
			WHEN LTrim(RTrim(@SearchClientID)) <> ''
				AND LTrim(RTrim(@SearchClientID)) <> '0'
				THEN LTrim(RTrim(@SearchClientID))
			ELSE AccMstr.ManagerCode
			END
		)
...
BEGIN
	PRINT 'CREATED USP_EX_GetAccountPagingAllAccounts';
END