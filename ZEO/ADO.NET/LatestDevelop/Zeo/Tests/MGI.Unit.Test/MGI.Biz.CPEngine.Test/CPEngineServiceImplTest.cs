...
namespace MGI.Biz.MoneyOrder.Test
{
	...
	public class CPEngineServiceImplTest : BaseClass_Fixture
	{
		...
		public void Can_Submit_CheckProcessing()
		{
			...
			CxeCheckSvc.Verify(moq => moq.Create(It.IsAny<CXEData.Transactions.Stage.Check>(), It.IsAny<string>()), Times.AtLeastOnce());
		...
		[Test]
		public void Can_Get_Check_Status_For_Pending_To_Declined()
		{
			long customerSessionId = 1000000003;
			string checkId = "1000000020";
			MGIContext mgiContext = new MGIContext();
			mgiContext.ChannelPartnerName = "Synovus";
			mgiContext.ChannelPartnerRowGuid = Guid.Parse("EC6AAAE3-2BA7-4E0B-898E-D296DB432C17");
			mgiContext.ChannelPartnerId = 33;
		...
		public void Can_Get_Check_Status_For_Declined()
		{
			...
			Assert.IsNotNull(status);
			CheckProcessor.Verify(moq => moq.Status(It.IsAny<long>(), It.IsAny<string>(), It.IsAny<MGIContext>()), Times.AtLeastOnce());
			CheckProcessor.Verify(moq => moq.Update(It.IsAny<CheckTrx>(), It.IsAny<MGIContext>()), Times.AtLeast(1));
		}
		...
		public void Can_ReSubmit_Check()
		{
			...
			Assert.True(status);
			CheckProcessor.Verify(moq => moq.Get(It.IsAny<long>()), Times.AtLeast(1));
		...
		public void Can_Get_Check_Declined_Trxn()
		{
			...
			Assert.IsNotNull(checkTransaction);
			CheckProcessor.Verify(moq => moq.Get(It.IsAny<long>()), Times.AtLeastOnce());
			CheckProcessor.Verify(moq => moq.Update(It.IsAny<CheckTrx>(), It.IsAny<MGIContext>()), Times.AtLeastOnce());
		}
        ...
        public void Can_Validate_Check_Fee_Lower()
        {
            long customerSessionId = 1000000003;
            CheckSubmission checkSubmission = GetCheckSubmission();
            checkSubmission.Amount = 0;
            MGIContext mgiContext = new MGIContext();
            mgiContext.ChannelPartnerName = "Synovus";
            mgiContext.ChannelPartnerRowGuid = Guid.Parse("EC6AAAE3-2BA7-4E0B-898E-D296DB432C17");
            mgiContext.ChannelPartnerId = 33;
        ...
        [Test]
        [ExpectedException(typeof(BizComplianceLimitException))]
        public void Can_Validate_Check_Fee_Higher__Limit()
        {
            long customerSessionId = 1000000003;
            CheckSubmission checkSubmission = GetCheckSubmission();
            checkSubmission.Amount = 200;
            MGIContext mgiContext = new MGIContext();
            mgiContext.ChannelPartnerName = "Synovus";
            mgiContext.ChannelPartnerRowGuid = Guid.Parse("EC6AAAE3-2BA7-4E0B-898E-D296DB432C17");
            mgiContext.ChannelPartnerId = 33;
		...
		[Test]
		public void Can_CheckProcessing_Decline()
		{
			...
			mgiContext.ChannelPartnerRowGuid = Guid.Parse("EC6AAAE3-2BA7-4E0B-898E-D296DB432C17");
			mgiContext.ChannelPartnerId = 33;
			mgiContext.TimeZone = "Eastern Time";
			mgiContext.Context = new Dictionary<string, object>();
			mgiContext.Context.Add("CheckStatus", "Decline");
			Check check = CPEngineService.Submit(checkSubmission, customerSessionId, mgiContext);
			Assert.IsNotNull(check);
			CxeCheckSvc.Verify(moq => moq.Create(It.IsAny<CXEData.Transactions.Stage.Check>(), It.IsAny<string>()), Times.AtLeastOnce());
		}
		...
		[Test]
		public void Can_CheckProcessing_Fail()
		{
			long customerSessionId = 1000000103;
			CheckSubmission checkSubmission = GetCheckSubmission();
			MGIContext mgiContext = new MGIContext();
			...
			mgiContext.TimeZone = "Eastern Time";
			mgiContext.Context = new Dictionary<string, object>();
			mgiContext.Context.Add("StatucCheck", "Failed");
			Check check = CPEngineService.Submit(checkSubmission, customerSessionId, mgiContext);
			Assert.IsNotNull(check);
			CxeCheckSvc.Verify(moq => moq.Create(It.IsAny<CXEData.Transactions.Stage.Check>(), It.IsAny<string>()), Times.AtLeastOnce());
		}
