...
namespace MGI.Unit.Test.MockData
{
	public class MockMoneyTransferProcessor : IntializMoqObject
	{
		...
		public CXNMoneyTransfer.IMoneyTransfer CreateInstanceOfCXNMoneyTransfer()
		{
			...
			(TransactionRequest transactionRequest, MGIContext mgiContext) =>
			{
				...
				if (transaction == null)
				{
					transaction = new Cxn.MoneyTransfer.Data.Transaction()
					{
						...
						ExpectedPayoutStateCode = "USA"
					};
					transaction.MetaData.Add("ExpectedPayoutCityName", "Test");
					transaction.MetaData.Add("ExpectedPayoutCity", "Calfornia");
					transaction.MetaData.Add("SenderStateCode", "United States");
			...
			{
				AccountNumber = "1592634875",
				sessionid = "1000000004",
				Sender = accounts.ToArray()
			});
