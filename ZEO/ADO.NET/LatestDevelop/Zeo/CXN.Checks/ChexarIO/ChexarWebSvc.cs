	...
	public class ChexarWebSvc : IChexarWebSvc
	{
		...
		public ChexarLogin GetChexarLogin(string baseURL, string companyId, string companyUname, string companyPwd, string employeeUname, string employeePwd)
		{
			...
			catch (Exception ex)
			{
				...
				#endregion
				HandleException(ex);
				throw new CheckException(CheckException.CHEXAR_LOGIN_FAILED, ex);

			}

			if (chexarLogin.CompanyToken == string.Empty)
				throw new CheckException(CheckException.CHEXAR_LOGIN_FAILED);
			...
			try
			{
				...
				{
					NLogger.Info(string.Format("Employee login failed: {0}", errorMessage, this.GetType().Name));
			...
			catch (Exception ex)
			{
				...
				HandleException(ex);
				throw new CheckException(CheckException.CHEXAR_EMPLOYEE_LOGIN_ERROR, ex);
		...
		public ChexarTicketStatus GetWaitTime(ChexarLogin login, int ticketNo)
		{
			...
			{
				HandleException(ex);
				throw new CheckException(CheckException.CHEXAR_GET_WAIT_TIME_FAILED, ex);
			}
		}
		...
		public bool CloseTransaction(ChexarLogin login, int invoiceNo, out string errorMessage)
		{
			...
			catch (Exception ex)
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				HandleException(ex);
				throw new CheckException(CheckException.CHEXAR_CLOSE_TRANSACTION_FAILED, ex);
			}
		}
		...
		public ChexarNewInvoiceResult CreateTransaction(ChexarLogin login, int badge, decimal amount, DateTime checkDate, int checkType, string checkNum, string routingNum, string accountNum, string micr, byte[] checkImgFront, byte[] checkImgBack, string checkImageFormat, byte[] checkImgFrontTIF, byte[] checkImgBackTIF, double[] geocodeLatLong, out int errorCode, out string errorMessage)
		{
			try
			{
				...
				if (geocodeLatLong == null || geocodeLatLong.Length != 2)
					...
					response = txnSvc.InvoiceDetailAddGeocode(login.CompanyToken, badge, login.IngoBranchId, login.EmployeeId, checkType, (double)amount, 0.00, string.Empty,
				...
				if (errorCode == 0)
				{
					...
					catch (Exception ex)
					{
						...
						if (ExceptionHelper.IsExceptionHandled(ex)) throw;
						HandleException(ex);
						throw new CheckException(CheckException.CHEXAR_TRANSACTION_CREATE_FAILED, ex);
					}
		...
		public ChexarMICRDetails GetMICRDetails(ChexarLogin login, int invoiceNum, out string errorMessage)
		{
			...
			catch (Exception ex)
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				HandleException(ex);
				throw new CheckException(CheckException.CHEXAR_GET_MICR_DETAILS_FAILED, ex);
			}
		}
		#endregion
		...
		public int RegisterNewCustomer(ChexarLogin login, ChexarCustomerIO customerInfo, out string errorMessage)
		{
			...
			try
			{
				...
				if (badge == 0)
					new ChexarProviderException(String.Format("{0:D4}", errorCode), errorMessage);
			}
			catch (Exception ex)
			{
				...
				new CheckException(CheckException.CHEXAR_REGISTER_FAILED, ex);
			}
			return badge;
		}
