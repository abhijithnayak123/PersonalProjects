...
namespace TCF.Zeo.Cxn.MoneyTransfer.WU.Impl
{
    public class SimulatorIO : BaseIO, IIO
    {
        ...
        {
            feeinquiryreply reply = new feeinquiryreply();
            reply.financials = new FeeInquiry.financials()
            {
            ...
            reply.promotions = new FeeInquiry.promotions()
            {
                promo_discount_amount = feeInquiryRequest.promotions.promo_discount_amount,

            };

            reply.payment_details = new FeeInquiry.payment_details()
            {
                exchange_rate = feeInquiryRequest.payment_details.exchange_rate,

            };
            reply.financials.taxes = new FeeInquiry.taxes()
            {
                state_tax = 1,
                municipal_tax = 1,
                county_tax = 1,
            };
            reply.fee_inquiry_message = new fee_inquiry_message()
            {
        ...
        public modifysendmoneysearchreply ModifySearch(modifysendmoneysearchrequest request, ZeoContext context)
        {
            ...
            ModifySendMoneySearch.payment_transaction payment = new WU.ModifySendMoneySearch.payment_transaction()
            {
                ...
                receiver = new ModifySendMoneySearch.receiver()
                {
                    name = new ModifySendMoneySearch.general_name()
                    {
                        name_type = WU.ModifySendMoneySearch.name_type.D,
                        first_name = "Chandler",
                        last_name = "Bing"
                    },
                    address = new ModifySendMoneySearch.address()
                ...
                payment_details = new ModifySendMoneySearch.payment_details()
                {
                    ...
                    transaction_type = ModifySendMoneySearch.transaction_type.WMN,
                    exchange_rate = 1.000,
                    money_transfer_type = ModifySendMoneySearch.money_transfer_type.WMN,
            ...
            searchreply.payment_transactions.payment_transaction = new ModifySendMoneySearch.payment_transaction[1] { payment };
            searchreply.foreign_remote_system = new ModifySendMoneySearch.foreign_remote_system()
            {
        ...
        public receivemoneypayreply ReceiveMoneyPay(receivemoneypayrequest receiveMoneyPayRequest, ZeoContext context)
        {
            ...
            {
                mtcn = Convert.ToString(Helper.GetLongRandomNumber(10)),
        ...
        {
           ReceiveMoneySearch.payment_transactions paymentTransactions = new ReceiveMoneySearch.payment_transactions();
        ...
        public sendmoneyvalidationreply SendMoneyValidate(sendmoneyvalidationrequest validationRequest, ZeoContext context)
        {
            ...
            reply.mtcn = Convert.ToString(Helper.GetLongRandomNumber(10));
            reply.new_mtcn = string.Format(Helper.GetLongRandomNumber(6) + reply.mtcn);
            reply.filing_date = string.Format(DateTime.Now.ToString("MM-dd"));
            reply.filing_time = string.Format(DateTime.Now.ToString("HHMMt") + " " + TimeZoneInfo.Utc.DisplayName);
