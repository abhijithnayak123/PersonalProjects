...
BEGIN TRY
		...
		SELECT 
			@countrycode = wr.PickupCountry, @receiverId = wr.WUReceiverID
		FROM 
			tWUnion_Trx wt
		INNER JOIN tWUnion_Receiver wr ON wr.WUReceiverID = wt.WUReceiverID
		WHERE 
			WUTrxId = @WUTrxId 
		...
		(
			SELECT dbo.ufn_IsOtherReceiverExists(@customerId, @receiverFname,  @receiverLname, @receiverSecondLname, @countrycode, @receiverId)
		)
	  ...
	  SELECT
			...
			,@isOtherReceiverExists AS IsOtherReceiverExists
