...
BEGIN TRY
	...
	FROM 
		...
		INNER JOIN tProductProcessorsMapping ppm ON cpp.ProductProcessorId = ppm.ProductProcessorsMappingID
		INNER JOIN tProcessors p ON p.ProcessorsID = ppm.ProcessorId
		INNER JOIN tProducts pr ON pr.ProductsID = ppm.ProductId
		INNER JOIN tChannelPartners cp ON cp.ChannelPartnerId = cpp.ChannelPartnerId
