...
BEGIN
	BEGIN TRY
		...
		FROM 
			...
			INNER JOIN tMasterCountries MC ON CM.MasterCountryId = MC.MasterCountriesPK 
			INNER JOIN tChannelPartners CP ON CP.ChannelPartnerPK = CM.ChannelPartnerId
		WHERE 
			CP.ChannelPartnerId = 34 
			AND IsActive = 1 
