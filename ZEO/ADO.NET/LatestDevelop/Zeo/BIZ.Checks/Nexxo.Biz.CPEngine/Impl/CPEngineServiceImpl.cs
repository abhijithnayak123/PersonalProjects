...
namespace MGI.Biz.CPEngine.Impl
{
    public class CPEngineServiceImpl : ICPEngineService
    {
        ...
        public Check Submit(CheckSubmission checkSubmission, long customerSessionId, MGIContext mgiContext)
        {
            try
            {
                ...
                else if (cxnStatus == CXNData.CheckStatus.Declined)
                {
                    ...
                    string key = BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + bizcpexception.AlloyErrorCode;
            ...
            catch (Exception ex)
            {
                ...
                throw new BizCPEngineException(BizCPEngineException.CHECK_SUBMIT_FAILED, ex);
            }
        }
        ...
        public Check GetStatus(long customerSessionId, string checkId, bool includeImage, MGIContext mgiContext)
        {
            try
            {
                ...
                if (checkStatus != (CXNData.CheckStatus)ptnrCheck.CXNState)
                {
                    ...
                    else if (checkStatus == CXNData.CheckStatus.Declined)
                    {
                        ptnrCheck.CXNState = ptnrCheck.CXEState = (int)CXEData.TransactionStates.Declined;
                        MGI.Core.Partner.Data.Language lang = MGI.Core.Partner.Data.Language.EN;
                        MGI.Biz.CPEngine.Data.BizCPEngineException bizcpexception = new BizCPEngineException(cxnCheck.DeclineCode);
                        MGI.Core.Partner.Data.Message _message = MessageStore.Lookup(session.AgentSession.Terminal.ChannelPartner.Id, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + bizcpexception.AlloyErrorCode, lang);
                        if (_message == null)
                        {
                            _message = MessageStore.Lookup(session.AgentSession.Terminal.ChannelPartner.Id, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + BizCPEngineException.UNHANDLED_DECLINE_CODE, lang);
                        }
                ...
                Check check = new Check
                {
                    ...
                    SubmissionDate = ptnrCheck.DTTerminalCreate,
                    StatusDescription = cxnCheck.WaitTime,
                    DeclineCode = Convert.ToInt32(cxnCheck.DeclineCode),
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.GET_STATUS_FAILED, ex);
        ...
        public bool Cancel(long customerSessionId, string checkId, MGIContext mgiContext)
        {
            try
            {
                ...
                CXNData.CheckStatus cxnStatus = checkProcessor.Status(ptnrCheck.CXNId, mgiContext.TimeZone, mgiContext);
                ...
                bool isCheckCancelled = checkProcessor.Cancel(ptnrCheck.CXNId, mgiContext.TimeZone, mgiContext);
                ...
                if (isCheckCancelled)
                {
                    _cxeCheckSvc.Update(ptnrCheck.CXEId, (int)cxeStatus, mgiContext.TimeZone);
            ...
            catch (Exception ex)
            {
                ...
                #endregion
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.CHECK_CANCEL_FAILED, ex);
            }
        }
        ...
        public void UpdateStatusOnRemoval(long customerSessionId, long checkId)
        {
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.CHECK_UPDATE_FAILED, ex);
            }
        }
        ...
        public bool CanResubmit(long customerSessionId, string checkId, MGIContext mgiContext)
        {
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.RESUBMIT_FAILED, ex);
            }
        }
        ...
        public void Commit(string checkId, long customerSessionId, MGIContext mgiContext)
        {
            try
            {
                ...
                checkProcessor.Commit(ptnrCheck.CXNId, mgiContext.TimeZone, mgiContext);

                // update stage check
                _cxeCheckSvc.Update(ptnrCheck.CXEId, (int)CXEData.TransactionStates.Committed, mgiContext.TimeZone);

                // commit check in cxe
                _cxeCheckSvc.Commit(ptnrCheck.CXEId);
            ...
            catch (Exception ex)
            {
                ...
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.COMMIT_FAILED, ex);
            }
        ...
        public List<string> GetCheckTypes(long customerSessionId, MGIContext mgiContext)
        {
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.GET_CHECK_TYPE_FAILE, ex);
            }
        }
        ...
        public BizCommon.TransactionFee GetFee(long customerSessionId, CheckSubmission checkSubmission, MGIContext mgiContext)
        {
            try
            {
                ...
                {
                    throw new BizComplianceLimitException(BizCPEngineException.ProductCode, BizComplianceLimitException.MINIMUM_LIMIT_FAILED, minimumAmount);
                }
                if (checkSubmission.Amount > maximumAmount)
                {
                    throw new BizComplianceLimitException(BizCPEngineException.ProductCode, BizComplianceLimitException.MAXIMUM_LIMIT_FAILED, maximumAmount);
                }
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.GET_FEE_FAILED, ex);
        ...
        public CheckTransaction GetTransaction(long agentSessionId, long customerSessionId, string checkId, MGIContext mgiContext)
        {
            try
            {
                ...
                if (cxnCheck.Status == CXNData.CheckStatus.Declined)
                {
                    ...
                    BizCPEngineException bizcpexception = new BizCPEngineException(cxnCheck.DeclineCode);
                    MGI.Core.Partner.Data.Message _message = MessageStore.Lookup(ChannelPartnerId, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + bizcpexception.AlloyErrorCode, lang);

                    if (_message == null)
                    {
                        _message = MessageStore.Lookup(ChannelPartnerId, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + BizCPEngineException.UNHANDLED_DECLINE_CODE, lang);
                    }
                ...
                return new CheckTransaction
                {
                    ...
                    DeclineErrorCode = Convert.ToInt32(cxnCheck.DeclineCode)
                };
            }
            catch (Exception ex)
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.GET_TRANSACTION_FAILED, ex);
        ...
        public string GetCheckFrankingData(long customerSessionId, long transactionId, MGIContext mgiContext)
        {
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.GET_CHECK_FRANK_FAILED, ex);
        ...
        public CheckProcessorInfo GetCheckProcessorInfo(long agentSessionId, MGIContext mgiContext)
        {
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCPEngineException(BizCPEngineException.CHECK_GET_FAILED, ex);
