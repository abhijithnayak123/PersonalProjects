...
namespace MGI.Channel.DMS.Server.Impl
{
	public partial class DesktopEngine : IAgentService
	{
		...
		[Transaction()]
		public Response AuthenticateSSO(AgentSSO ssoAgent, string channelPartner, string terminalName, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			...
			response.Result = getAgentSession(agent, channelPartner, terminalName, mgiContext);
			return response;
		}
		...
		[Transaction()]
		public Response UpdateSession(long agentSessionId, Terminal terminal, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			BizPartnerData.Terminal bizTerminal = Mapper.Map<BizPartnerData.Terminal>(terminal);
			Response response = new Response();
			response.Result = AgentService.UpdateSession(agentSessionId, bizTerminal, context);
			return response;
		}
