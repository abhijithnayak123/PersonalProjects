	...
	public class CustomerProspectService : ICustomerProspectService
	{
		...
		public string SaveProspect(long agentSessionId, SessionContext sessionContext, ProspectDTO prospectDTO, MGIContext mgiContext)
		{
            ...
            catch(Exception ex)
            {
                ...
                throw new BizCustomerException(BizCustomerException.PROSPECT_SAVE_FAILED, ex);
            }
		}
		...
		public void SaveProspect(long agentSessionId, SessionContext sessionContext, long alloyId, ProspectDTO prospectDTO, MGIContext mgiContext)
		{
            try
            {
                ...
                if (prospectDTO == null)
                    throw new BizCustomerException(BizCustomerException.PROSPECT_SAVE_FAILED);
		...
		public ProspectDTO GetProspect(long agentSessionId, SessionContext sessionContext, long alloyId, MGIContext mgiContext)
		{
			try
            {
                ...
                if(prospect == null)
                    throw new BizCustomerException(BizCustomerException.PROSPECT_FETCH_FAILED);
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCustomerException(BizCustomerException.PROSPECT_FETCH_FAILED, ex);
            }
		}
		...
		public string ConfirmIdentity(long agentSessionId, long customerSessionId, bool status, MGIContext mgiContext)
		{
            ...
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new BizCustomerException(BizCustomerException.CONFIRM_IDENTITY_FAILED, ex);
            }
		}
			...
			if (customer != null)
			{
				...
				if (ptnrAcct != null)
				{
					...
					if (checkAccount != null)
					{
						...
						checkAccount.Address2 = prospect.Address2;
						...
						if (prospect.GovernmentId != null && prospect.GovernmentId.IdType != null && prospect.GovernmentId.IdType.State != null)
						{
				...
				if (!string.IsNullOrEmpty(fundProviderName))
				{
					...
					if (ptnrAcct != null)
					{
						...
						if (cardAccount != null)
						{
							...
							cardAccount.MothersMaidenName = prospect.MothersMaidenName;
							cardAccount.Email = prospect.Email;
		...
		private NexxoIdTypeDto GetIdType(long channelPartnerId, Identification prospectID)
		{
			...
			if (prospectID != null && !string.IsNullOrEmpty(prospectID.Country) && !string.IsNullOrEmpty(prospectID.IDType))
			{
				...
				if (idType == null)
					throw new BizCustomerException(BizCustomerException.INVALID_CUSTOMER_DATA_ID_TYPE_NOT_FOUND);
			}
			return idType;
		}
