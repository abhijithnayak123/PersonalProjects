	...
	public class FISIOImpl
	{
		...
		public FIS.Data.FISAccount SearchCustomerBySSN(MGIContext mgiContext)
		{
            ...
            try
            {
                ...
                fisAccount = SearchCustomerBySSNResponseMapper(responseData, mgiContext.ChannelPartnerId);
		...
		{
            List<FIS.Data.FISAccount> fisAccounts = new List<FISAccount>();
			...
			try
            {
                ...
                fisAccounts = FetchAllResponseMapper(customerInfoList, cxnContext.ChannelPartnerId);
		...
		public void UpdateCustomerProfile(FIS.Data.FISAccount customerprofile, MGIContext context)
		{
            ...
            catch (Exception ex)
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                HandleException(ex);
                throw new CustomerException(CustomerException.UPDATE_CUSTOMER_FAILED, ex);
            }
		...
		public bool CreateMiscAccount(FIS.Data.FISAccount customerprofile, MGIContext mgiContext)
		{
            ...
            catch (Exception ex)
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                HandleException(ex);
                throw new CustomerException(CustomerException.CREATE_MISC_CUSTOMER_FAILED, ex);
            }
		}
		...
		private FISAccount SearchCustomerBySSNResponseMapper(CICustTaxNbrSrchResData responseData, long channelPartnerId)
		{
			...
			if (responseData.CICustInfoLst != null)
			{
				...
				if (responseData.CICustInfoLst.Count() > 1)
                    throw new CustomerException(CustomerException.MULTIPLE_ACCOUNT_FOUND, null);
		...
		private void RaiseException(IEnumerable<Error> errormessage)
		{
            ...
            //As discussed, we will be showing the first excpetion in pop up as per story AL-7329.
            var error = errormessage.FirstOrDefault();

            if(error != null)
            {
                 code = "CI" + error.ErrorNumber;
                 message = error.ErrorMessage;
            }

            throw new FISProviderException(code, message, null);
		}
		...
		private void LogFISError(CIOpenMiscAcctMtvnSvcReq request, string bankId, IEnumerable<Error> errorMessages, MGIContext mgiContext)
		{
			if (errorMessages.Any())
			{
				...
				if (!string.IsNullOrWhiteSpace(mgiContext.BranchId))
				{
					branchId = mgiContext.BranchId;
				}
		...
		private FISCredential GetFISCredential(long channelPartnerId, string bankId)
		{
			...
			if (credential == null)
            { throw new CustomerException(CustomerException.CREDENTIALS_NOT_FOUND, null); }
			...
			{
                throw new CustomerException(CustomerException.APPLICATION_INFO_ERROR, null);
			}
        ...
        {
            Exception fisProviderException = ex as FISProviderException;
            if (fisProviderException != null)
            {
                FISProviderException fisException = fisProviderException as FISProviderException;
                string errorCode = Convert.ToString(fisException.ProviderErrorCode);
                string errorMsg = Convert.ToString(fisException.Message);

                throw new FISProviderException(errorCode, errorMsg, null);
            }
            Exception faultException = ex as FaultException;
            if (faultException != null)
            {
                throw new FISProviderException(FISProviderException.PROVIDER_FAULT_ERROR, string.Empty, faultException);
            }
            Exception endpointException = ex as EndpointNotFoundException;
            if (endpointException != null)
            {
                throw new FISProviderException(FISProviderException.PROVIDER_ENDPOINTNOTFOUND_ERROR, string.Empty, endpointException);
            }
            Exception commException = ex as CommunicationException;
            if (commException != null)
            {
                throw new FISProviderException(FISProviderException.PROVIDER_COMMUNICATION_ERROR, string.Empty, commException);
            }
            Exception timeOutException = ex as TimeoutException;
            if (timeOutException != null)
            {
                throw new FISProviderException(FISProviderException.PROVIDER_TIMEOUT_ERROR, string.Empty, timeOutException);
            }
        }
