...
namespace MGI.Biz.MoneyTransfer.Impl
{
	public partial class MoneyTransferEngine : IMoneyTransferEngine
	{
		...
		public Data.FeeResponse GetFee(long customerSessionId, FeeRequest feeRequest, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetFee", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetFee-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETFEE_FAILED, ex);
			}
		...
		public bool IsSWBState(long customerSessionId, string stateCode, MGIContext mgiContext)
		{
           #region AL-3370 Transactional Log User Story
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "IsSWBState", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in IsSWBState-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_ISSWBSTATE_FAILED, ex);
			}
           #region AL-3370 Transactional Log User Story
		...
		public ValidateResponse Validate(long customerSessionId, ValidateRequest validateRequest, MGIContext mgiContext)
		{
			try
			{
				...
				if (validateRequest.TransferType == TransferType.SendMoney)
				{
					...
					{
						throw new BizComplianceLimitException(BizMoneyTransferException.MoneyTransferProductCode, BizComplianceLimitException.MINIMUM_LIMIT_FAILED, minimumAmount);
					}
					if (currentTransactionAmount > maximumAmount)
					{
						throw new BizComplianceLimitException(BizMoneyTransferException.MoneyTransferProductCode, BizComplianceLimitException.MAXIMUM_LIMIT_FAILED, maximumAmount);
					}
				}
				...
				if (customer.GovernmentId != null)
				{
					...
					if (govtIDInfo != null)
					{
						...
						cxnValidateRequest.PrimaryCountryOfIssue = !string.IsNullOrWhiteSpace(govtIDInfo.Country) ? govtIDInfo.Country : string.Empty;
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Validate", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Validate -MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_VALIDATE_FAILED, ex);
			}
		}
		...
		public int Commit(long customerSessionId, long ptnrTransactionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Commit", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Commit-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_COMMIT_FAILED, ex);
			}
		}
		...
		public long AddReceiver(long customerSessionId, bizReceiver receiver, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "AddReceiver", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in AddReceiver-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_ADDRECEIVER_FAILED, ex);
			}
		...
		public long EditReceiver(long customerSessionId, bizReceiver receiver, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "EditReceiver", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in EditReceiver-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_EDITRECEIVER_FAILED, ex);
			}
		...
		public bizReceiver GetReceiver(long customerSessionId, long Id, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetReceiver", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetReceiver-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETRECEIVER_FAILED, ex);
			}
		}
		...
		public List<bizReceiver> GetReceivers(long customerSessionId, string lastName, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetReceivers", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetReceivers-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETRECEIVERS_FAILED, ex);
			}
		}
		...
		public List<bizReceiver> GetActiveReceivers(long customerSessionId, string lastName, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetActiveReceivers", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetActiveReceivers-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETACTIVERECEIVER_FAILED, ex);
			}
		}
		...
		public List<bizReceiver> GetFrequentReceivers(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetFrequentReceivers", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetFrequentReceivers-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETFREQUENTRECEIVERS_FAILED, ex);
			}
		...
		public void DeleteFavoriteReceiver(long customerSessionId, long receiverId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "DeleteFavoriteReceiver", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in DeleteFavoriteReceiver-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_DELETEFAVORITERECEIVER_FAILED, ex);
			}
		}
		...
		public MoneyTransferTransaction Get(long customerSessionId, TransactionRequest request, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Get", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Get-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GET_FAILED, ex);
			}
		}
		...
		public MoneyTransferTransaction GetReceiverLastTransaction(long customerSessionId, long receiverId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetReceiverLastTransaction", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetReceiverLastTransaction-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETRECEIVERLASTTRANSACTION_FAILED, ex);
			}
		}
		...
		public bool UpdateAccount(long customerSessionId, string WUGoldCardNumber, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "UpdateAccount", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in UpdateAccount-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(MGI.Common.Util.NexxoUtil.GetProductCode(mgiContext.ProductType), BizMoneyTransferException.MONEYTRANSFER_UPDATEACCOUNT_FAILED, ex);
			}
		...
		public CardDetails WUCardEnrollment(long customerSessionId, PaymentDetails paymentDetails, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "WUCardEnrollment", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in WUCardEnrollment-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(MGI.Common.Util.NexxoUtil.GetProductCode(mgiContext.ProductType), BizMoneyTransferException.MONEYTRANSFER_WUCARDENROLLMENT_FAILED, ex);
			}
		}
		...
		public CardLookupDetails WUCardLookup(long customerSessionId, CardLookupDetails LookupDetails, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "WUCardLookup", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in WUCardLookup-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(MGI.Common.Util.NexxoUtil.GetProductCode(mgiContext.ProductType), BizMoneyTransferException.MONEYTRANSFER_WUCARDLOOKUP_FAILED, ex);
			}
		}
		...
		public bool GetAccount(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetAccount", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetAccount-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETACCOUNT_FAILED, ex);
			}
		}
		...
		public bizAccount DisplayWUCardAccountInfo(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "DisplayWUCardAccountInfo", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in DisplayWUCardAccountInfo-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(MGI.Common.Util.NexxoUtil.GetProductCode(mgiContext.ProductType), BizMoneyTransferException.MONEYTRANSFER_DISPLAYWUCARDACCOUNTINFO_FAILED, ex);
			}
		}
		...
		public void Cancel(long customerSessionId, long transactionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Cancel", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Cancel-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_CANCEL_FAILED, ex);
			}
		}
		...
		public void AddPastReceivers(long customerSessionId, string cardNumber, MGIContext mgiContext)
		{
			try
			{
               #region AL-3370 Transactional Log User Story
				...
				{
				    //TODO - Abhi - Call the method to update the CounterId to IsAvailable false and insert the record in
                    //tCustomerSessionCounterIdDetails table.
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "AddPastReceivers", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in AddPastReceivers-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_ADDPASTRECEIVERS_FAILED, ex);
			}
		...
		public CardInfo GetCardInfo(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetCardInfo", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetCardInfo-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(MGI.Common.Util.NexxoUtil.GetProductCode(mgiContext.ProductType), BizMoneyTransferException.MONEYTRANSFER_GETCARDINFO_FAILED, ex);
			}
		}
		...
		public string GetStatus(long customerSessionId, string confirmationNumber, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetStatus", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetStatus-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETSTATUS_FAILED, ex);
			}
		}
		...
		public SearchResponse Search(long customerSessionId, SearchRequest request, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Search", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Search-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_SEARCH_FAILED, ex);
			}
		}
		...
		public ModifyResponse StageModify(long customerSessionId, ModifyRequest modifySendMoney, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "StageModify", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in StageModify-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_STAGEMODIFY_FAILED, ex);
			}
		}
		...
		public long StageRefund(long customerSessionId, RefundRequest refundRequest, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "StageRefund", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in StageRefund-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_STAGEREFUND_FAILED, ex);
			}
		}
		...
		public void AuthorizeModify(long customerSessionId, ModifyRequest modifySendMoney, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "AuthorizeModify", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in AuthorizeModify-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_AUTHORIZEMODIFY_FAILED, ex);
			}
		}
		...
		public int Modify(long customerSessionId, ModifyRequest modifySendMoney, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Modify", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Modify-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_MODIFY_FAILED, ex);
			}
		}
		...
		public string Refund(long customerSessionId, RefundRequest refundSendMoney, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "Refund", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in Refund-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_REFUND_FAILED, ex);
			}
		}
		...
		private void UpdateReceiverName(long customerSessionId, cxnTransaction cxnTransaction, MGIContext mgiContext)
		{
			...
			{
				if (!string.IsNullOrEmpty(cxnTransaction.ReceiverFirstName))
					reciever.FirstName = cxnTransaction.ReceiverFirstName;
				if (!string.IsNullOrEmpty(cxnTransaction.ReceiverLastName))
					reciever.LastName = cxnTransaction.ReceiverLastName;
				if (!string.IsNullOrEmpty(cxnTransaction.Receiver.MiddleName))
					reciever.MiddleName = cxnTransaction.Receiver.MiddleName;
				if (!string.IsNullOrEmpty(cxnTransaction.ReceiverSecondLastName))
					reciever.SecondLastName = cxnTransaction.ReceiverSecondLastName;
			}
		...
		public List<DeliveryService> GetDeliveryServices(long customerSessionId, DeliveryServiceRequest request, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetDeliveryServices", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetDeliveryServices-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETDELIVERYSERVICES_FAILED, ex);
			}
		...
		public List<Field> GetProviderAttributes(long customerSessionId, AttributeRequest attributeRequest, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetProviderAttributes", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in GetProviderAttributes-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETPROVIDERATTRIBUTES_FAILED, ex);
			}
		}
		...
		public void UpdateTransactionStatus(long customerSessionId, long transactionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "UpdateTransactionStatus", AlloyLayerName.BIZ, ModuleName.SendMoney,
									  "Error in UpdateTransactionStatus-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_UPDATETRANSACTIONSTATUS_FAILED, ex);
			}
		}
	}
}
