...
namespace MGI.Biz.FundsEngine.Impl
{
	...
	public class FundsEngineImpl : IFundsEngine
	{
			...
			Mapper.CreateMap<MGI.Cxn.Fund.Data.FundTrx, BizFunds>()
				...
				.AfterMap((s, d) =>
				{
					if (s.TransactionType != null)
					{
						...
						if (s.TransactionType.ToString() == MGI.Cxn.Fund.Data.TransactionType.Credit.ToString())
							fundType = FundType.Credit;
						else if (s.TransactionType.ToString() == MGI.Cxn.Fund.Data.TransactionType.Debit.ToString())
							fundType = FundType.Debit;
						else if (s.TransactionType.ToString() == MGI.Cxn.Fund.Data.TransactionType.AddOnCard.ToString())
		...
		public long Add(long customerSessionId, BizFundsAccount fundsAccount, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Info<object>(customerSessionId, fundsAccount, "Add", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", mgiContext);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.ADD_FUNDS_FAILED, ex);
			}
		}
		...
		public bool AuthenticateCard(long customerSessionId, string cardNumber, string authenticationInfo, string encryptionKey, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Info<string>(customerSessionId, cardNumber, "AuthenticateCard", AlloyLayerName.BIZ, ModuleName.Funds,
											"Error in MGI.Biz.FundsEngine.Impl", mgiContext);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_AUTHENTICATECARD_FAILED, ex);
			}
		}
		...
		public long Withdraw(long customerSessionId, BizFunds bizFunds, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Withdraw", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_WITHDRAW_FAILED, ex);
			}
		}
		...
		public long Activate(long customerSessionId, BizFunds bizFunds, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Activate", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_ACTIVATE_FAILED, ex);
			}
		}
		...
		public long Load(long customerSessionId, BizFunds bizFunds, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Load", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_LOAD_FAILED, ex);
			}
		}
		...
		public int Commit(long customerSessionId, long cxeTrxId, MGIContext mgiContext, string cardNumber = "")
		{
			try
			{
				...
				//timestamp changes
				CXEFundsService.Commit(ptnrTrx.CXEId, mgiContext.TimeZone);

				CXEFundsService.Update(ptnrTrx.CXEId, TransactionStates.Committed, mgiContext.TimeZone);
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Commit", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);
		...
		public Data.CardInfo GetBalance(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "GetBalance", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_BALANCE_RETRIVEL_FAILED, ex);
			}
		}
		...
		public BizFundsAccount GetAccount(long customerSessionId, MGIContext mgiContext)
		{
			...
			catch (Exception ex)
			{
				...
				var exp = ex as ProviderException;
				if (exp != null && exp.ProductCode == VisaProviderException.ProductCode && exp.ProviderErrorCode == VisaProviderException.PROVIDER_COMMUNICATION_ERROR)
				{
					cardInfo.Balance = decimal.MinValue;
				}
				else
				{
					MongoDBLogger.Error<long>(customerSessionId, "GetAccount", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);
		...
		public BizFunds Get(long customerSessionId, long Id, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Get", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_TRANSACTION_RETRIVEL_FAILED, ex);
			}
		}
		...
		public BizCommon.TransactionFee GetFee(long customerSessionId, decimal amount, FundType fundsType, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "GetFee", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_FEE_FAILED, ex);
			}
		}
		...
		public long UpdateAmount(long cxeFundTrxId, decimal amount, long customerSessionId, FundType fundType, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "UpdateAmount", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_UPDATE_FAILED, ex);
			}
		}
		#endregion
		...
		public decimal GetMinimumLoadAmount(long customerSessionId, bool initialLoad, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "GetMinimumLoadAmount", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_GETMINIMUM_FAILED, ex);
			}
		}
		...
		public void Cancel(long customerSessionId, long fundsId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "Cancel", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_CANCEL_TRANSACTION_FAILED, ex);
			}
		}
		...
		public List<Data.TransactionHistory> GetTransactionHistory(long customerSessionId, Data.TransactionHistoryRequest request, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "GetTransactionHistory", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_TRANSACTION_HISTORY_RETRIVEL_FAILED, ex);
			}
		}
		...
		public bool CloseAccount(long customerSessionId, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "CloseAccount", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);


				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_ACCOUNT_CLOSURE_FAILED, ex);
			}
		}
		...
		public bool UpdateCardStatus(long customerSessionId, Data.CardMaintenanceInfo cardMaintenanceInfo, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "UpdateCardStatus", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_UPDATE_STATUS_FAILED, ex);
			}
		}
		...
		public bool ReplaceCard(long customerSessionId, Data.CardMaintenanceInfo cardMaintenanceInfo, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<long>(customerSessionId, "ReplaceCard", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_CARD_REPLACE_FAILED, ex);
			}
		}
		...
		public List<MGI.Biz.FundsEngine.Data.ShippingTypes> GetShippingTypes(MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<MGIContext>(mgiContext, "GetShippingTypes", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_SHIPPING_TYPE_RETRIVEL_FAILED, ex);
			}
		}
		...
		public double GetShippingFee(Data.CardMaintenanceInfo cardMaintenanceInfo, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<Data.CardMaintenanceInfo>(cardMaintenanceInfo, "GetShippingFee", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_SHIPPING_FEE_RETRIVEL_FAILED, ex);
			}
		}
		...
		public long AssociateCard(long customerSessionId, BizFundsAccount fundsAccount, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<BizFundsAccount>(fundsAccount, "AssociateCard", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_CARD_ASSOCIATION_FAILED, ex);
			}
		}
		...
		public double GetFundFee(Data.CardMaintenanceInfo cardMaintenanceInfo, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<Data.CardMaintenanceInfo>(cardMaintenanceInfo, "GetFundFee", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_CARDMAINTENANCE_FEE_RETRIVEL_FAILED, ex);
			}
		}
		...
		public long IssueAddOnCard(long customerSessionId, BizFunds funds, MGIContext mgiContext)
		{
			...
			{
				MongoDBLogger.Error<Data.Funds>(funds, "IssueAddOnCard", AlloyLayerName.BIZ, ModuleName.Funds,
											 "Error in MGI.Biz.FundsEngine.Impl", ex.Message, ex.StackTrace);

				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizFundsException(BizFundsException.FUNDS_ORDER_COMPANION_FAILED, ex);
			}
		}
			...
			RequestType requestType, MGIContext mgiContext, decimal amount)
		{
			...
			var maxException = BizComplianceLimitException.MAXIMUM_LIMIT_FAILED;
			...
			if ((requestType == RequestType.Credit || requestType == RequestType.Debit))
			{
				...
				if (requestType == RequestType.Credit)
				{
					...
					maxException = BizComplianceLimitException.MAXIMUM_LIMIT_FAILED;
				}
				...
				{
					throw new BizComplianceLimitException(BizFundsException.FundsProductCode, maxException, maximumAmount);
				}
			}
		}
