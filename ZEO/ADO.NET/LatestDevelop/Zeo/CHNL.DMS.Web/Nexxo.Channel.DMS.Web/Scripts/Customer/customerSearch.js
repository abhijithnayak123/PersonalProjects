var jqgridfunction = function () {
	...
	jQuery(document).ready(function () {
		if (($("#SSN").val() != "" && $("#SSN").val() != "___-__-____") || $("#MaskCardNumber").val() != "" || $("#LastName").val() != "" || $('#PhoneNumber').val() != "" || ($('#DateOfBirth').val() != "" && $('#DateOfBirth').val() != "__ / __ / ____")) {
	...
	function customerIDGenerator(cellvalue, options, rowObject) {
		...
		cellvalue = "<a id='customersearchgridrow' href='#' data-href='" + validateCustomer + "/" + options.rowId + "'>" + cellvalue + "</a>";
		return cellvalue;
	}
}

var xhr;
function ValidateCustomerStatusAndId(validateCustomerUrl) {
    var ValidateCustomerUrl = validateCustomerUrl;
    ...
    xhr = $.ajax({
        ...
        dataType: "json",
        type: "GET",
        url: ValidateCustomerUrl,
        contentType: "application/json; charset=UTF-8",
        processData: true,
        success: function (data) {
            if (data)
            {
                // Added if condition for exception Handling to handle the error from action method - "ValidateCustomerStatusAndId".
...
$(document).ready(function () {
	...
	$("#customersearchgridrow").live('click', function (e) {
	    ...
	    ValidateCustomerStatusAndId(dataHref);
	})
	$('#LastName').focus();
});
