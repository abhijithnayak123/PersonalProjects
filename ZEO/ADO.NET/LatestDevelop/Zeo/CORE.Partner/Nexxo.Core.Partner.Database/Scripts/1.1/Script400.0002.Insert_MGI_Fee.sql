...
-- Description:		Script for insert MGI Fee for Money order,fund and check
-- Rally ID:		
--===========================================================================================

DECLARE @PartnerPK UNIQUEIDENTIFIER,
		@FeeMin DECIMAL
SELECT @PartnerPK = rowguid FROM tChannelPartners WHERE id = 1

--MoneyOrder Fee		
IF NOT EXISTS (SELECT [ChannelPartnerPK] FROM [dbo].[tChannelPartnerFees_MoneyOrder] WHERE [ChannelPartnerPK] = @PartnerPK)
BEGIN
...
--Funds Fee
IF NOT EXISTS (SELECT [ChannelPartnerPK] FROM [dbo].[tChannelPartnerFees_Funds] WHERE [ChannelPartnerPK] = @PartnerPK)
BEGIN
...
IF NOT EXISTS (SELECT [ChannelPartnerPK] FROM [dbo].[tChannelPartnerFees_Check] WHERE [ChannelPartnerPK] = @PartnerPK)
BEGIN
