...
FROM 
	tAccounts ptnrAcnt
	INNER JOIN tCCIS_Account cxnAcnt ON cxnAcnt.CCISAccountID = ptnrAcnt.CXNId
	AND ProviderId = @carverProviderId AND PartnerAccountNumber IS NOT NULL 
	INNER JOIN tCustomers cxeCust ON cxeCust.CustomerID = ptnrAcnt.CXEId
GO
...
FROM 
	tAccounts ptnrAcnt
	INNER JOIN tFIS_Account cxnAcnt ON cxnAcnt.FISAccountID = ptnrAcnt.CXNId
