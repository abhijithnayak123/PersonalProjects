...
-- Description:	 Staged Dates are being printed on Committed Receipts .
-- Jira ID:		<AL-7079>
-- ================================================================================

/****** Object:  View [dbo].[vTransactionHistory]    Script Date: 5/31/2016 1:28:24 PM ******/
IF  EXISTS (SELECT 1 FROM sys.views WHERE object_id = OBJECT_ID(N'[dbo].[vTransactionHistory]'))
...
FROM   
	...
	INNER JOIN tCustomers sC ON sC.CustomerID = tPC.CustomerID 
	LEFT JOIN sChxr_Trx sCH ON sCH.ChxrTrxID = tC.CXNId AND tA.ProviderId = 200
	LEFT JOIN tTxn_Check_Stage	sCT ON sCT.CheckID = tC.CXEId 
	...
	LEFT JOIN tTxn_Check_Stage sCS ON sCS.CheckID = tC.CXEId 
...
SELECT   
 tF.TxnPK AS rowguid,   
	...
	--WHEN 3 THEN 'Companion Card Order'   
...
FROM  
	...
	LEFT JOIN sVisa_Trx sV ON sV.VisaTrxID = tF.CXNId AND tA.ProviderId = 103
	LEFT JOIN sVisa_Account sVA ON sVA.VisaAccountPK = sV.AccountPK
	LEFT JOIN tCustomers sCC on tF.AddOnCustomerId = sCC.CustomerID
WHERE   
