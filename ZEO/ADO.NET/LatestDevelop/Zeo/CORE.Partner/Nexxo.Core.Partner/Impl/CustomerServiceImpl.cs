	...
	public class CustomerServiceImpl : ICustomerService
	{
		...
		public Customer Create(Customer customer)
		{
			if (customer.ChannelPartnerId == Guid.Empty)
				throw new PartnerCustomerException(PartnerCustomerException.CUSTOMER_CHANNELPARTNER_NOT_FOUND);
		...
		public Customer LookupByCXNAccountId(long cxnAccountId, int providerId)
		{
			try
			{
				...
				if (accounts.Count > 1)
					throw new PartnerCustomerException(PartnerCustomerException.CUSTOMER_MULTIPLE_ACCOUNT_FOUND, null);
		...
		public void SaveGroupSetting(CustomerGroupSetting _CustomerGroupSetting)
		{
			...
			{
                throw new PartnerCustomerException(PartnerCustomerException.CUSTOMER_SAVE_GROUP_SETTINGS_FAILED, ex);
			}
		}
	}
}
