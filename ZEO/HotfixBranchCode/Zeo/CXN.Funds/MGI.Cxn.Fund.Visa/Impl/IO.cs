	...
	public class IO : IIO
	{
		...
		public Data.CardInfo GetCardInfoByProxyId(string proxyId, Credential credential)
		{
			...
			catch (CommunicationException ex)
			{
				...
				throw new FundException(FundException.COMMUNICATION_ERROR, "VISA DPS is not available at this time.", ex);
			}
		...
		public CardPurchaseResponse IssueCard(Account account, double initialLoadAmount, Data.CardInfo cardInformation, Credential credential)
		{
			...
			try
			{
				...
				{
					throw new VisaProviderException(VisaProviderException.CARD_REGISTRATION_ERROR,
		...
		public Data.CardBalance GetBalance(long aliasId, Credential credential)
		{
			...
			catch (EndpointNotFoundException ex)
			{
				...
				throw new FundException(FundException.COMMUNICATION_ERROR, "Unable to connect to the Visa server", ex);
			}
			catch (CommunicationException ex)
			{
				//AL-3372 Transactional Log User Story
			...
			try
			{
				...
				if (response != null)
				{
					...
					if (response.CardholderJointAccountListResponse != null && response.CardholderJointAccountListResponse.JointCardholders.Count > 0)
					{
						...
						//if (response.CardholderJointAccountListResponse.JointCardholders.Count > 0)
						//{
						//	foreach (var cardHolder in response.CardholderJointAccountListResponse.JointCardholders)
						//	{
						//		if (cardHolder != null && cardHolder.CardIdentifier != null)
						//		{
						//			string companionAliasId = Convert.ToString(cardHolder.CardIdentifier.AliasId);
						//			joinHolders.Add(companionAliasId);
						//		}
						//	}
						//	cardInformation.AliasId = aliasId;
						//}
		...
		private static void HandleException(FaultException ex)
		{
			if (ex is FaultException<VisaDetail>)
			{
				VisaDetail visaDetail = ((FaultException<VisaDetail>)(ex)).Detail;
				if (visaDetail != null && visaDetail.VisaErrors != null && visaDetail.VisaErrors.Any())
				{
					...
					throw new VisaProviderException(errorCode, visaError.Description);
				}
			}
			else
			{
				throw new VisaProviderException(VisaProviderException.CARD_REGISTRATION_ERROR, ex.Message);
			}
		}
