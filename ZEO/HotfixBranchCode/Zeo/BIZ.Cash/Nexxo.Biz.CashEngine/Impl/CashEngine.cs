	...
	public class CashEngine : ICashEngine
	{
		...
		private CustomerSession GetCustomerSession(long customerSessionId)
		{
			try
			{
				...
				if (session == null)
				{
                    NLogger.Info("----------SESSION-NULL----------");
					throw new BizCashEngineException(BizCashEngineException.INVALID_SESSION, "Invalid Session");
				}
			...
			{
                NLogger.Error("INVALID_SESSION" + ex.InnerException);
				throw new BizCashEngineException(BizCashEngineException.INVALID_SESSION, "Invalid Session", ex);
			}
		}
