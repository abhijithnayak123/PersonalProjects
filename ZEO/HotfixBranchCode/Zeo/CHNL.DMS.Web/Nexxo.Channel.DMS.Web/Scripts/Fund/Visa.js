$(document).ready(function () {
	...
	$('#InitialLoad').keyup(function (e) {
		var loadAmount = $(this).val();
		var loadDecimal = loadAmount.indexOf('.');
...
function EncryptCardNo() {
	...
	if (result != null) {
		...
		showExceptionPopupMsg("Please enter a valid card number");
	}
}
	...
	$.ajax({
		...
		success: function (data) {
			...
			if (data.success == 'false') {
				showExceptionPopupMsg(data.Err);
				RemovePopUp();
			}
			else {
				if (data == true) {
	...
	$.ajax({
		...
		success: function (data) {
			if (data.Data.IsPrimaryCardHolder) {
				var customerLookUpURL = CustomerLookUP_Url + '?IsException=false&ExceptionMsg=""&isPrimaryCardHolder=' + data.Data.IsPrimaryCardHolder;
