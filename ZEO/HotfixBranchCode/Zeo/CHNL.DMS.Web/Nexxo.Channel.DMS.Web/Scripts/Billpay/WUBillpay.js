...
$(document).ready(function () {
	...
	/********************************************************************************/
	//        User Story Number: US1763 | Web | TA3928
	//        Developed by: Sunil Shetty
	//        Date: 14.01.2014
	//        Purpose: Modified Select Biller field not allow the teller to manually enter a biller name on this field, and then tab off the field without selecting a biller name returned in the DAS Inquiry reply		
	...
	$('#BillAmount').blur(function () {
		...
		if (this.value && !isNaN(this.value) && billerName && accountNumber) {
			showSpinners($("#loading"));
	...
	if (billerName && (typeof val == "undefined")) {
		$.ajax({
			...
			success: function (data) {
				...
				if (data.Message) {
					$(".bill_pay_desc").show();
					$("#billerMessage").html(data.Message);
				}
				else {
					$(".bill_pay_desc").hide();
				}
...
function PopulateBillPayeeAccount(selectedBillPayee) {
	$.ajax({
		...
		success: function (data) {
			...
			if (data != null && data != "") {
				if (data.ProviderName != null && data.ProviderName != "") {
					$('#BillerId').val(data.BillerId);
					if (data.BillerName) {
						$('#BillerName').val(data.BillerName);
						$('#hdnBillerName').val(data.BillerName);
					}
