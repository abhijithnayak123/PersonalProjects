	...
	public partial class WUGateway : IMoneyTransfer
	{
		...
		public List<Receiver> GetFrequentReceivers(long CustomerId)
		{
			...
			catch (Exception ex)
			{
				...
				ErrorMessage = string.Format("Error while invoking WU.GetFrequentReceivers for Customer ID : {0}", CustomerId);
				...
				else
				{
					...
					if (existingReceiver != null)
					{
						...
						if (!receiverExisted)
						{
							...
							throw new MoneyTransferException(MoneyTransferException.RECEIVER_ALREADY_EXISTED, "Receiver already existed with first name and last name combination.");
					}
		...
		public bool Commit(long transactionId, MGIContext mgiContext)
		{
			...
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Error in Commit Money Transfer Transaction with Status {0} : {1}", (string.IsNullOrEmpty(mgiContext.SMTrxType)) ? mgiContext.RMTrxType : mgiContext.SMTrxType, ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
				NLogger.Error("Error :" + ex.Message + " Stack Trace:" + ex.StackTrace);
				...
				string transactionType = mgiContext.SMTrxType == null ? mgiContext.RMTrxType : mgiContext.SMTrxType;

				//AL-3370 Transactional Log User Story
				MongoDBLogger.Error<string>(Convert.ToString(transactionId), "Commit", AlloyLayerName.CXN, ModuleName.MoneyTransfer,
					"Error in Commit - MGI.Cxn.MoneyTransfer.WU.Impl.WUGateway", ex.Message, ex.StackTrace);

				ErrorMessage = string.Format("Error in Commit Money Transfer Transaction for {0} with Status {1} : {2}", receiverName, transactionType, ex.Message);
				throw new Exception(ErrorMessage, ex);
			}
		}
		...
		public List<MoneyTransfer.Data.MasterData> GetBannerMsgs(MGIContext mgiContext)
		{
			if (string.IsNullOrEmpty(mgiContext.WUCounterId))
				throw new MoneyTransferException(MoneyTransferException.COUNTERID_NOT_FOUND, "Western Union counter Id is not available or has not been correctly setup");
		...
		private void UpdateReceiveMoneyTransaction(long transactionId, receivemoneysearchreply reply, MGIContext mgiContext)
		{
			...
			{
				throw new InvalidOperationException("This transaction is already paid");
			}
		...
		private object getFromContext(Dictionary<string, object> context, string key)
		{
			if (!context.ContainsKey(key) || context[key] == null)
			{ throw new Exception(string.Format("Unable to find {0} in context ", key)); }
		...
		private long CreateReceiveMoneyTransaction(long accountId, MGIContext mgiContext)
		{
			...
			if (string.IsNullOrEmpty(mgiContext.TimeZone))
				throw new Exception("Time zone not provided");
			...
			{
				throw new Exception("Error in Western Union Transaction Create", ex);
			}
		...
		public bool GetPastReceivers(long customerSessionId, string cardNumber, MGIContext mgiContext)
		{
			...
			catch (MoneyTransferException ex)
			{
				...
				throw new MoneyTransferException(ex.MinorCode, ex.Message, ex.InnerException);
			}
			catch (WUCommonException ex)
			{
				NLogger.Error("Error :" + ex.Message + " Stack Trace:" + ex.StackTrace);
				...
				ErrorMessage = string.Format("Error in GetPastReceivers Method {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
				...
				ErrorMessage = string.Format("EXCEPTION while getting past receivers details {0}", ex.Message);
				throw new Exception(ErrorMessage, ex);
			}
		}
		...
		public bool UseGoldcard(long accountId, string WUGoldCardNumber, MGIContext mgiContext)
		{
			...
			try
			{
				...
				if (!(cxncardlookupDetails.Sender[0].FirstName.Trim().ToLower().Equals(wuAccount.FirstName.Trim().ToLower())
					...
					throw new MoneyTransferException(MoneyTransferException.CUSTOMER_NAME_NOT_MATCH, "The My WU customer name does not match the Alloy Customer Profile name");
			...
			catch (MoneyTransferException ex)
			{
				...
				MongoDBLogger.ListError<string>(details, "UseGoldcard", AlloyLayerName.CXN, ModuleName.SendMoney,
			...
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Error in Card lookup {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
				...
				ErrorMessage = string.Format("EXCEPTION while Enrolling Goldcard Details {0}", ex.Message);
				throw new Exception(ErrorMessage, ex);
			}
		}
		...
		public string GetStatus(string confirmationNumber, MGIContext mgiContext)
		{
			...
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Pay Status Not Found  {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
				...
				throw new Exception("Pay Status Not Found " + ex.Message);
			}
		}
		...
		public void Modify(long transactionId, MGIContext mgiContext)
		{
			...
			try
			{
				...
				{
					throw new MoneyTransferException(MoneyTransferException.MODIFY_TRANSACTION_NOT_ALLOWED, "Transaction not allowed");
				}
				else
				{
					throw new MoneyTransferException(MoneyTransferException.TRANSACTION_ALREADY_PAID, "Transaction already PAID");
				}
			}
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Error while Modify Send Money Transaction : {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
		...
		private CxnData.SearchResponse SearchModify(CxnData.SearchRequest searchRequest, MGIContext mgiContext)
		{
			...
			{
				NLogger.Error("Error :" + ex.Message + " Stack Trace:" + ex.StackTrace);
				ErrorMessage = string.Format("Error while Search Modify Send Money Transaction : {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
		...
		private void UpdateSendMoneyModifyTransaction(long transactionId, modifysendmoneyreply response, MGIContext mgiContext)
		{
			...
			{
				throw new Exception("Error in Western Union Transaction Modify", ex.InnerException);
			}
		}
		...
		private CxnData.SearchResponse SearchRefund(CxnData.SearchRequest searchRefundRequest, MGIContext mgiContext)
		{
			...
			{
				NLogger.Error("Error :" + ex.Message + " Stack Trace:" + ex.StackTrace);
				ErrorMessage = string.Format("Error in Search Refund Transaction with Status {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
		...
		public string Refund(MGI.Cxn.MoneyTransfer.Data.RefundRequest refundRequest, MGIContext mgiContext)
		{
			...
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Error in Commit Refund Transaction with Status {0}", ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (Exception ex)
			{
		...
		private long CreateSendMoneyTransaction(WUTransaction WUTrxlog, MGIContext mgiContext)
		{
			...
			{
				throw new Exception("Error in Western Union Create Modify Transaction", ex);
			}
			return WUTrxlog.Id;
		}
		...
		private long CreateSendMoneyRefundTransaction(WUTransaction trx, MGIContext mgiContext)
		{
			...
			{
				throw new Exception("Error in Western Union Create Refund Transaction", ex);
			}
			return WUTrxlog.Id;
		}
		...
		private void UpdateSendMoneyRefundTransaction(long transactionId, SendMoneyRefund.refundreply refundsendresponse, MGIContext mgiContext)
		{
			...
			{
				throw new Exception("Error in Western Union Refund Transaction Update", ex.InnerException);
			}
		}
		...
		private string GetTimeZone(MGIContext mgiContext)
		{
			if (string.IsNullOrEmpty(mgiContext.TimeZone))
			{
				throw new MoneyTransferException(MoneyTransferException.INVALID_DICTIONARY_KEY, "TimeZone not found in the context");
			}
		...
		private static void ValidateDeliveryServices(DeliveryServiceRequest request, MGIContext mgiContext)
		{
			...
			{
				throw new MoneyTransferException(MoneyTransferException.DESTINATION_COUNTRY_CODE_NOT_FOUND, "Destination Country Code not found in the context");
			}

			if (!request.MetaData.ContainsKey("State"))
			{
				throw new MoneyTransferException(MoneyTransferException.DESTINATION_COUNTRY_CODE_NOT_FOUND, "State not found in the context");
			}

			if (!request.MetaData.ContainsKey("StateCode"))
			{
				throw new MoneyTransferException(MoneyTransferException.DESTINATION_COUNTRY_CODE_NOT_FOUND, "State code not found in the context");
			}

			if (!request.MetaData.ContainsKey("City"))
			{
				throw new MoneyTransferException(MoneyTransferException.DESTINATION_COUNTRY_CODE_NOT_FOUND, "City found in the context");
			}
			...
			if (request.Type == DeliveryServiceType.Option)
			{
				...
				{
					throw new MoneyTransferException(MoneyTransferException.SVC_CODE_NOT_FOUND, "Delivery service not found in the context");
				}
			}
		}
		...
		public FeeResponse GetFee(FeeRequest feeRequest, MGIContext mgiContext)
		{
			...
			catch (WUCommonException ex)
			{
				...
				ErrorMessage = string.Format("Error while get fee for {0} : {1}", transferAmount, ex.Message);
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (TimeoutException timeoutEx)
			{
				//AL-3370 Transactional Log User Story
				MongoDBLogger.Error<FeeRequest>(feeRequest, "GetFee", AlloyLayerName.CXN, ModuleName.SendMoney,
					"Error in GetFee - MGI.Cxn.MoneyTransfer.WU.Impl.WUGateway", timeoutEx.Message, timeoutEx.StackTrace);

				ErrorMessage = string.Format("Error while get fee for {0}", transferAmount);
			...
			catch (WebException webEx)
			{
				//AL-3370 Transactional Log User Story
				MongoDBLogger.Error<FeeRequest>(feeRequest, "GetFee", AlloyLayerName.CXN, ModuleName.SendMoney,
					"Error in GetFee - MGI.Cxn.MoneyTransfer.WU.Impl.WUGateway", webEx.Message, webEx.StackTrace);
		...
		public ValidateResponse Validate(ValidateRequest validateRequest, MGIContext mgiContext)
		{
			...
			catch (WUCommonException ex)
			{
				...
				string[] _minorCode = new string[] { "5050", "7490", "6008", "6009", "0425", "0415" };
				...
				//		END AL-471 changes
				ErrorMessage = string.Format("Error while Validate {1} Transaction : {0}", ex.Message, validateRequest.TransferType == MoneyTransferType.Send ? "Send Money " : "Receive Money");
				throw new WUCommonException(ex.MinorCode, ErrorMessage, ex);
			}
			catch (MoneyTransferException ex)
			{
		...
		private void CheckCounterId(MGIContext mgiContext)
		{
			...
			{
				if (string.IsNullOrEmpty(mgiContext.WUCounterId))
					throw new WUCommonException(MoneyTransferException.COUNTERID_NOT_FOUND, "Western Union counter Id is not available or has not been correctly setup.");
			}
		}
		...
		private int GetExceptionMessage(string MinorCode)
		{
			...
			{
				case "5050":
					return MoneyTransferException.MISSING_SSN_ITIN;
				case "7490":
					return MoneyTransferException.REQUIRES_SSN_ITIN;
				case "6008":
				case "6009":
					return MoneyTransferException.INVALID_SOCIAL_SECURITY_NUMBER;
				case "0425":
					return MoneyTransferException.TRANSACTION_REQUIRES_ADDITIONAL_CUSTOMER_INFORMATION; //AL-2967
				case "0415":
					return MoneyTransferException.DO_TRANSACTION_REQUIRES_ADDITIONAL_CUSTOMER_INFORMATION; //AL-3175
				default:
					return 0;
			}
		}
		...
		public string GetDeliveryServiceTransalation(string serviceName, string language)
		{
			...
			{
				throw new MoneyTransferException(MoneyTransferException.TRANSALATED_DELIVERY_SERVICE_NOT_EXITS, "Transalated delivery service not exits");
			}
		...
		public string GetCountryTransalation(string countryCode, string language)
		{
			...
			try
			{
				...
				if (country == null)
					throw new MoneyTransferException(MoneyTransferException.TRANSALATED_COUNTRY_NOT_EXITS, "Transalated country not exits ");
			...
			catch (Exception ex)
			{
				...
				throw new MoneyTransferException(MoneyTransferException.TRANSALATED_COUNTRY_NOT_EXITS, ex);
			}
			return transalatedName;
		}
