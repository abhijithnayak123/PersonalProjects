...
namespace MGI.Unit.Test.MockClasses
{
	public class MockCommonClass : IntializMoqObject
	{
		...
		public PTNRContract.IManageUsers CreateInstanceOfManageUsers()
		{
			...
			ManageUserService.Setup(moq => moq.GetManagers()).Returns(userDetails);
			...
			ManageUserService.Setup(moq => moq.GetUserRoles()).Returns(userRole);
			...
			ManageUserService.Setup(moq => moq.GetUserStatuses()).Returns(userStatus);
			...
			ManageUserService.Setup(moq => moq.SaveUserLocation(It.IsAny<PTNRData.UserLocation>())).Returns(
				(PTNRData.UserLocation userLocation) =>
				{
					userLocations.Add(userLocation);
					return userLocation.AgentId;
				});

			ManageUserService.Setup(moq => moq.SearchUsers(It.IsAny<PTNRData.UserSearchCriteria>())).Returns(userDetails);
			...
			ManageUserService.Setup(moq => moq.GetUser(It.IsAny<string>(), It.IsAny<long>())).Returns(
				(string userName, long channelPartnerId) => 
				{
					...
					{
						throw new PTNRContract.ChannelPartnerException(PTNRContract.ChannelPartnerException.USER_NOT_FOUND, "User not found");
					}
					if (userName == "Nitish")
					{
						return null;
		...
		public IClientCustomerService CreateInstanceOfClientCustomerService()
		{
			...
			CxnClientCustomerService.Setup(moq => moq.Add(It.IsAny<CustomerProfile>(), It.IsAny<MGIContext>())).Returns(long.MaxValue);

			CxnClientCustomerService.Setup(moq => moq.FetchAll(It.IsAny<Dictionary<string, object>>(), It.IsAny<MGIContext>())).Returns(
				new List<CustomerProfile>() { new CustomerProfile(){ CustInd = true, ClientID = "1111111111111111" } });
			...
			CxnClientCustomerService.Setup(moq => moq.GetClientProfileStatus(It.IsAny<long>(), It.IsAny<MGIContext>())).Returns(
				...
				{
					if (mgiContext.ChannelPartnerId == 33)
					{
			...
			CxnClientCustomerService.Setup(moq => moq.Update(It.IsAny<string>(), It.IsAny<CustomerProfile>(), It.IsAny<MGIContext>()));
