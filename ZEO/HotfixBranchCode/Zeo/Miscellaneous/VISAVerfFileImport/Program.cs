    ...
    class Program
    {
        ...
        private static int AssociateAccounts(DataSet sourceds)
        {
            ...
            using (sqlconn = new SqlConnection(cxnconnectionstring))
            {
                ...
                string visasql = "select v.VisaAccountID  as VISAAccountId,t.TSysAccountID as TsysAccountId from tVisa_Account v inner join tTSys_Account t on v.ProxyId = RIGHT(REPLICATE('0',19) + t.ExternalKey,19) where t.activated = 1  order by v.DTServerCreate desc";
            ...
            {
                DataRow dr = sourceds.Tables["PTNRAccounts"].Select("Providerid = 102 and CXNID=" + row["TsysAccountId"]).FirstOrDefault();
                if (dr == null)
                {
        ...
        private static DataSet ReadAllAccounts()
        {
            ...
            using (sqlconn = new SqlConnection(cxnconnectionstring))
            {
                ...
                foreach (var group in duplicates)
                {
                    ...
                    WriteToConsole("INFO\t:\tException Row - Tsys\t:\tFound " + group.Count().ToString() + " record(s) in TsysAccount for ExternalKey " + group.Key.ToString(), true);
                }
