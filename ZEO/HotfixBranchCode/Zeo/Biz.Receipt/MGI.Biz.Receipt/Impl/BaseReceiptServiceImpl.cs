...
namespace MGI.Biz.Receipt.Impl
{
	public class BaseReceiptServiceImpl : IReceiptService
	{
		...
		public virtual List<Data.Receipt> GetCheckReceipt(long agentSessionId, long customerSessionId, long transactionId, bool isReprint, MGIContext mgiContext)
		{
			...
			CXNCheckData.CheckTrx cxnCheck = _GetCheckProcessor(channelpartner.Name).Get(check.Id);
		...
		public virtual List<Data.Receipt> GetFundsReceipt(long agentSessionId, long customerSessionId, long transactionId, bool isReprint, MGIContext mgiContext)
		{
			...
			string transactionTags = appendFundsTags(agentSessionId, fundtrx, GetCxnContext(mgiContext));
		...
		{
			throw new BizPtrnContract.BizReceiptException(100, "Use partner specific implementation", null);
		}
		//TODO Change Message
		...
		public virtual List<Data.Receipt> GetFundsActivationReceipt(long agentSessionId, long customerSessionId, List<BizPtrnData.Transactions.Funds> fundTransactions, MGIContext mgiContext)
		{
			...
			decimal latestCardBalance = _GetProcessor(channelPartnerName).GetBalance(cardAccount.Id, GetCxnContext(mgiContext), out processorResult).Balance;
		...
		{
			//PTNRData.Terminal terminal = _terminalSvc.Lookup(trx.CustomerSession.AgentSession.TerminalPK);
			...
			DateTime trxDate = trx == null ? GetTransactionSummaryDate(customerSession) : trx.DTTerminalCreate;
		...
		private DateTime GetTransactionSummaryDate(PTNRData.CustomerSession customerSession)
		{
			...
			if (transactionDetails != null)
			{
				...
				summaryDate = trx.Transaction.DTTerminalCreate;
			}
		...
		private MGI.Core.Partner.Data.CustomerSession GetCustomerSession(long customerSessionId)
		{
			try
			{
				...
				if (session == null)
				{
					throw new BizPtrnContract.BizCustomerException(BizPtrnContract.BizCustomerException.INVALID_CUSTOMER_ID);
				}
		...
		private MGIContext GetCxnContext(MGIContext mgiContext)
		{
			if (mgiContext.ProcessorId == 0)
				throw new BizReceiptException(BizReceiptException.PROCESSOR_NOT_SET, "Processor Not Set in the Context");
