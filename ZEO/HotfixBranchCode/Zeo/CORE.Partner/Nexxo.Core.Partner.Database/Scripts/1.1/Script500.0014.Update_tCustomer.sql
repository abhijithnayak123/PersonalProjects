...
FROM 
	tAccounts ptnrAcnt
	INNER JOIN stCCISAccount cxnAcnt ON cxnAcnt.CCISAccountID = ptnrAcnt.CXNId
	AND ProviderId = @carverProviderId AND PartnerAccountNumber IS NOT NULL 
	INNER JOIN sCustomer cxeCust ON cxeCust.CustomerID = ptnrAcnt.CXEId
GO
...
FROM 
	tAccounts ptnrAcnt
	INNER JOIN stFISAccount cxnAcnt ON cxnAcnt.FISAccountID = ptnrAcnt.CXNId
