...
CREATE TRIGGER [dbo].[trgLocationCounterIdInsertUpdate] ON [dbo].[tLocations] 
AFTER INSERT,UPDATE
AS
BEGIN
	...
	SELECT @LocationId=rowguid, @LocationIdentifier=LocationIdentifier,@Count=NoOfCounterIds FROM inserted
	
	if(@LocationIdentifier  IS NOT NULL and @LocationIdentifier <> '')
	BEGIN
