		...
		$.ajax({
			...
			success: function (data, textStatus, jqXHR) {
				if (data.ErrorNo != undefined) {
					...
					showExceptionPopupMsg($("#MoneyOrderCheckPrintError").val() + ", " + data.ErrorDetails);
					return;
				}
			...
			error: function (data, errmessage, errdetails) {
				...
				if (data.ErrorNo != undefined) {
					...
					$('#btnPrint').removeAttr('disabled');
					$('#btnPrint').removeClass('DisableButtons');
					showExceptionPopupMsg($("#ConnectivityError").val());
					return;
				}
			},
			timeout: 30000
...
function ScanMoneyOrder(baseURL, moneyOrderId) {
	...
	$.ajax({
		...
		success: function (data) {
			if (data.ErrorNo != undefined) {
				hideSpinner();
				showExceptionPopupMsg($("#MoneyOrderScanError").val() + ", " + data.ErrorDetails);
				return;
			}
			...
			if (data.ScanCheckResult.MicrAuxillatyOnUSField == "") {
				showExceptionPopupMsg("The Money Order Check Number was not scanned correctly. Please retry the scan process. If you continue to see this error please try scanning a different Money Order Check.");
			}
		...
		error: function (data, errmessage, errdetails) {
			...
			if (data.ErrNo != undefined) {
				...
				showExceptionPopupMsg($("#MoneyOrderScanError").val());
				return;
			}
		},
...
function ProcessMoneyOrderPopup() {
	...
	$ScanMoneyOrder.dialog({
		autoOpen: false,
		title: "MGiAlloy - Scan Money Order Check",
		width: 630,
		draggable: false,
		modal: true,
		minHeight: 260,
...
function MoneyOrderScanConfirmPopup() {
	...
	$MoneyOrderConfirm.dialog({
		autoOpen: false,
		title: "MGiAlloy - Scan Money Order Check Confirm",
		width: 650,
		draggable: false,
		modal: true,
		minHeight: 600,
...
function MoneyOrderPrintPopup(checkNumber, frontImage, backImage, npsId, accountNumber, routingNumber, micr) {
	...
	$PrintMoneyOrder.dialog({
		autoOpen: false,
		title: "MGiAlloy - Print Money Order Check",
		width: 630,
		draggable: false,
		modal: true,
		minHeight: 260,
...
function MoneyOrderPrintConfirmationPopup(checkNumber, frontImage, backImage, accountNumber, routingNumber, micr,npsId,amount) {	
	...
	$confirm.dialog({
		...
		closeOnEscape: false,
		open: function (event, ui) {
			$confirm.load(MoneyOrderPrintConfirmPopupURL, MoneyOrder, function () {
				$('#btnPrint').focus();
			});
		}
	});
