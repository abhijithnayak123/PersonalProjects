...
$(document).ready(function () {
	...
	$("#SSN").blur(function () {
		...
		if (ssn != "" && ssn.length == 11) {
			ssn = maskedValue + ssn.substring(7, ssn.length).replace("-", "");
			$("#SSN").val(ssn);
		}
		else {
			$("#SSN").val(''); ;
	...
	jQuery("#jqTable").jqGrid(
        {
        	...
        	colModel: [
                ...
                { name: "Date of Birth", index: "DateOfBirth", width: 110, align: "center", jsonmap: "dob" },
                { name: "ID Number", index: "ID", width: 110, align: "center", jsonmap: "customerid" },
