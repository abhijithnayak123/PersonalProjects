...
namespace MGI.Providers.WU.DASMonitor
{
	public class WesternUnionIO : IDASServices
	{
        ...
        string _ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["PCATDBConnection"].ConnectionString;
        string _CXNConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["CXNDBConnection"].ConnectionString;
		...
		public void PopulateWUQQCompanyNames()
		{
			try
			{
				...
				if (wuBillers.Count != 0)
				{
					...
					try
					{
						con = _GetConnection(_CXNConnectionString);
						con.Open();
					...
					if (_result == 1)
					{
						...
						try
						{
							...
							con.Open();
                            using  (SqlConnection cxnconnection = _GetConnection(_CXNConnectionString))
                            {
                                CXNDatabase = cxnconnection.Database;   
                            }
