...
namespace MGI.Providers.MG.Monitor
{
	public class MoneyGramMonitor
	{
        ...
        string CXNConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["CXNDBConnection"].ConnectionString;
        string ConnectionString = System.Configuration.ConfigurationManager.ConnectionStrings["PCATDBConnection"].ConnectionString;
		...
		string CXNDatabase = System.Configuration.ConfigurationManager.AppSettings.Get("CXNDatabase");
		...
		private static void BulkInsert(CTResponse codeTable, List<StateRegulator> stateRegulators)
		{
            ...
            string connectionString = System.Configuration.ConfigurationManager.ConnectionStrings["CXNDBConnection"].ConnectionString;
		...
		public void PopulateBillers()
		{
			try
			{
				...
				if (mgBillers.Count != 0)
				{
					...
					try
					{
						con = GetConnection(CXNConnectionString);
						con.Open();
					...
					if (result == 1)
					{
						...
						try
						{
							...
							foreach (string channelPartnerID in partnerIDs)
							{
								SqlCommand cmd = new SqlCommand("USP_PopulateMGramCatalog", con);
								cmd.CommandType = System.Data.CommandType.StoredProcedure;
								cmd.Parameters.AddWithValue("@DATABASENAME", CXNDatabase);
