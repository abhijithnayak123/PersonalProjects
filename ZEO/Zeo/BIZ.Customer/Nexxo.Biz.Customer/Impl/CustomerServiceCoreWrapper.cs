	...
	public class CustomerServiceCoreWrapper : BizCustomerService
	{
		...
		public CustomerServiceCoreWrapper()
		{
			...
			Mapper.CreateMap<CXECustomer, Biz.Customer.Data.CustomerProfile>()
				...
				.ForMember(x => x.IDIssuingCountry, opt => opt.MapFrom(y => _ptnrIdTypeService.Find(y.ChannelPartnerId, y.GovernmentId.IdTypeId).Country))
				...
				.ForMember(x => x.IDIssuingState, opt => opt.MapFrom(y => _ptnrIdTypeService.Find(y.ChannelPartnerId, y.GovernmentId.IdTypeId).State))
				...
				if (!string.IsNullOrWhiteSpace(criteria.CardNumber))
				{
					...
					if (cxnAccountId == 0)
					{
						...
						if (cxnAccountId == 0 && cxnAccountId != -1)
							throw new FundException(FundException.CARD_MAPPING_ERROR);
					}
		...
		public CustomerSessionDTO InitiateCustomerSession(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			...
			if (agentSessionId == 0 && (mgiContext.ApplicationName == "DMS-Server"))
				throw new BizCustomerException(BizCustomerException.INVALID_SESSION_ID);
		...
		public void SaveCustomer(long agentSessionId, long alloyId, CustomerDTO customerDTO, MGIContext mgiContext)
		{
			...
			if (customerDTO.ID != null)
			{
				...
				if (!string.IsNullOrEmpty(customerDTO.ID.Country) && !string.IsNullOrEmpty(customerDTO.ID.IDType))
				{
					...
					if (idType == null)
						throw new MGI.Biz.Customer.Data.BizCustomerException(BizCustomerException.INVALID_CUSTOMER_DATA_ID_TYPE_NOT_FOUND);
				}
		...
		public List<CustomerDTO> CustomerLookUp(long agentSessionId, Dictionary<string, object> customerLookUpCriteria, MGIContext mgiContext)
		{
			...
			if (mgiContext.IsCompanionSearch && customerLookUpCriteria.ContainsKey("SSN"))
			{
				string ssn = customerLookUpCriteria["SSN"].ToString();
				CXECustomerSearchCriteria cxeCriteria = new CXECustomerSearchCriteria()
				{
					SSN = ssn
				};
				List<CXECustomer> cxeCustomer = new List<CXECustomer>();
				//Lookup cxe customer based on search criteria 
				cxeCustomer = _cxeCustomerService.Lookup(cxeCriteria);

				foreach (var item in cxeCustomer)
				{
					if (item.ChannelPartnerId == mgiContext.ChannelPartnerId)
					{
						CXECustomer customer = cxeCustomer.Find(a => a.ChannelPartnerId == item.ChannelPartnerId && a.ProfileStatus != ProfileStatus.Closed);
						if (customer != null)
						{
							customers.Add(customer);
						}
			...
			{
				customerdtolist = Mapper.Map<List<CXECustomer>, List<CustomerDTO>>(customers);
			}
		...
		public long GetAnonymousUserPAN(long agentSessionId, long channelPartnerId, string firstName, string lastName, MGIContext mgiContext)
		{
			...
			if (cxeCustomer == null || cxeCustomer.Id < 1)
                throw new BizCustomerException(BizCustomerException.CUSTOMER_FETCH_FAILED);
		...
		public void UpdateCustomerToClient(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			...
			customerProfile.BranchId = mgiContext.BranchId;
