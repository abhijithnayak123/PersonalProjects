...
namespace MGI.Channel.DMS.Server.Impl
{
	...
	.ForMember(x => x.ID, o => o.Ignore());
		...
		public Response Save(long agentSessionId, ProspectDTO prospectDto, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
            ...
            response.Result = Convert.ToString(alloyId);
		...
		public Response Save(long agentSessionId, long alloyId, ProspectDTO prospectDto, MGIContext mgiContext)
		{
		...
		[Transaction]
		public Response NexxoActivate(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			SharedSessionContextDTO contextDTO = Mapper.Map<SessionContextDTO, SharedSessionContextDTO>(GetSessionContext(Convert.ToString(agentSessionId)));
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
		...
		public Response ClientActivate(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
		...
		public Response UpdateCustomer(long agentSessionId, long alloyId, ProspectDTO prospectDTO, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
            Response response = new Response();
		...
		[Transaction(Spring.Transaction.TransactionPropagation.NotSupported)]
		public Response UpdateCustomerToClient(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
		...
		[Transaction(Spring.Transaction.TransactionPropagation.NotSupported)]
        public Response CustomerSyncInFromClient(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
        ...
        public Response InitiateCustomerSession(long agentSessionId, CustomerAuthentication authentication, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
			...
			Response res = LookupForPAN(Convert.ToInt64(sessionDTO.CustomerSessionId), mgiContext);

			FundsAccountDTO fundAccount = null;
			if (res.Error == null)
				fundAccount = res.Result as FundsAccountDTO;
			...
			Response wucardResponse = GetWUCardAccount(long.Parse(sessionDTO.CustomerSessionId), mgiContext);
            sessionDTO.Customer.IsWUGoldCard = (bool)wucardResponse.Result;
		...
		public Response Lookup(long customerSessionId, long alloyId, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			...
			Response res = LookupForPAN(Convert.ToInt64(customerSessionId), mgiContext);
			...
			Response wucardResponse = GetWUCardAccount(customerSessionId, mgiContext);
            customer.IsWUGoldCard = (bool)wucardResponse.Result;
		...
		public Response SearchCustomers(long agentSessionId, CustomerSearchCriteriaDTO searchCriteriaDTO, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			...
			List<CustomerSearchResultDTO> customerResults = Mapper.Map<List<bizCustomerSearchResult>, List<CustomerSearchResultDTO>>(results);

            return new Response() { Result = customerResults };
		}
        ...
        public Response GetProspect(long agentSessionId, long alloyId, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
            ...
            return new Response() { Result = prospectDTO };
		}
        ...
        public Response RecordIdentificationConfirmation(long customerSessionId, string agentId, bool IdentificationStatus, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			...
			response.Result = BizProspectService.ConfirmIdentity(customerSessionId, long.Parse(agentId), IdentificationStatus, context);
        ...
        public Response Get(long agentSessionId, string Phone, string PIN, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
            ...
            response.Result = GetCustomerMapper(customer);
		...
		public Response ValidateSSN(long agentSessionId, string SSN, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			...
			response.Result = BizCustomerService.ValidateSSN(agentSessionId, SSN, context);
        ...
        public Response ValidateCustomer(long agentSessionId, ProspectDTO prospect, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
            ...
            response.Result = BizCustomerService.ValidateCustomer(agentSessionId, bizCustomer, context);
        ...
        public Response GetAnonymousUserPAN(long agentSessionId, long channelPartnerId, MGI.Channel.DMS.Server.Data.MGIContext mgiContext)
		{
            ...
            response.Result = BizCustomerService.GetAnonymousUserPAN(agentSessionId, channelPartnerId, "unregistered", "customer", context);
		...
		/// <returns></returns>
		[Transaction(ReadOnly = true)]
        public Response CustomerLookUp(long agentSessionId, Dictionary<string, object> customerLookUpCriteria, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
            ...
            response.Result = Mapper.Map<List<bizCustomer>, List<ProspectDTO>>(bizCustomers);

            return response;
		}

		[Transaction(ReadOnly = true)]
		public Response ValidateCustomerStatus(long agentSessionId, long alloyId, MGIContext mgiContext)
		{
			mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
            Response response = new Response();
        ...
        public Response CanChangeProfileStatus(long agentSessionId, long userId, string profileStatus, MGIContext mgiContext)
		{
            ...
            response.Result = BizCustomerService.CanChangeProfileStatus(agentSessionId, userId, profileStatus, context);
