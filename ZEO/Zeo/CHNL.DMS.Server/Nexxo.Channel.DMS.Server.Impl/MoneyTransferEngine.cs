...
using dmsBannerMessage = MGI.Channel.Shared.Server.Data.BannerMessage;
using bizBannerMessage = MGI.Biz.MoneyTransfer.Data.BannerMessage;
...
namespace MGI.Channel.DMS.Server.Impl
{
	public partial class DesktopEngine : IMoneyTransferService, IMoneyTransferSetupService
	{
		...
		public Response AddReceiver(long customerSessionId, dmsReceiver receiver, MGIContext mgiContext)
		{
			...
			response.Result = SharedEngine.AddReceiver(customerSessionId, receiver, context);
			return response;
		}
		...
		public Response EditReceiver(long customerSessionId, dmsReceiver receiver, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.EditReceiver(customerSessionId, receiver, context);
			return response;
		}
		...
		public Response GetFrequentReceivers(long customerSessionId, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetFrequentReceivers(customerSessionId, context);
			return response;
		}
		...
		public Response GetReceiver(long customerSessionId, long Id, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetReceiver(customerSessionId, Id, context);
			return response;
		}
		...
		public Response GetFee(long customerSessionId, SharedData.FeeRequest feeRequest, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrFee(customerSessionId, feeRequest, context);
			return response;
		...
		public Response GetAgentXfer(long agentSessionId, MGIContext mgiContext)
		{
			...
			response.Result = GetAgentDetails(agentSessionid, context); 
			return response;
		}
		...
		public Response IsSWBStateXfer(long customerSessionId, string locationState, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = bizMoneyTransferEngine.IsSWBState(customerSessionId, locationState, context);
			return response;
		}
		...
		public Response ValidateTransfer(long customerSessionId, SharedData.ValidateRequest validateRequest, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.ValidateXfr(customerSessionId, validateRequest, context);
			return response;
		}

		[Transaction(ReadOnly = true)]
		public Response GetDeliveryServices(long customerSessionId, SharedData.DeliveryServiceRequest request, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrDeliveryServices(customerSessionId, request, context);
			return response;
		}

		[Transaction(ReadOnly = true)]
		public Response GetXfrProviderAttributes(long customerSessionId, SharedData.AttributeRequest attributeRequest, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrProviderAttributes(customerSessionId, attributeRequest, context);
			return response;
		}
		...
		public Response GetMoneyTransferDetailsTransaction(long customerSessionId, long transactionId, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrTransaction(customerSessionId, transactionId, context);
			return response;
		}
		...
		public Response GetReceivers(long customerSessionId, string lastName, MGIContext mgiContext)
		{
			...
			#endregion
			IList<dmsReceiver> dmsReceivers = Mapper.Map<IList<bizReceiver>, IList<dmsReceiver>>(receivers);
			response.Result = dmsReceivers;
			return response;
		}
		...
		public Response GetReceiver(long customerSessionId, string fullName, MGIContext mgiContext)
		{
			...
			dmsReceiver dmsReceiver = Mapper.Map<bizReceiver, dmsReceiver>(receiver);
			response.Result = dmsReceiver;
            #region AL-3370 Transactional Log User Story
		...
		public Response Get(long customerSessionId, SharedData.ReceiveMoneyRequest request, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			...
			SharedData.MoneyTransferTransaction moneyTransferTrxn = Mapper.Map<BizData.MoneyTransferTransaction, SharedData.MoneyTransferTransaction>(receivetransaction);
			response.Result = moneyTransferTrxn;
		...
		public Response WUCardEnrollment(long customerSessionId, dmsPaymentDetails paymentDetails, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
		...
		public Response WUCardLookup(long customerSessionId, SharedData.CardLookupDetails wucardlookupreq, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
		...
		public Response GetWUCardAccount(long customerSessionId, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = bizMoneyTransferEngine.GetAccount(customerSessionId, context);
			return response;
		}
		...
		public Response GetCardInfoXfer(long customerSessionId, MGIContext mgiContext)
		{
			...
			SharedData.CardInfo sharedCardInfo = Mapper.Map<SharedData.CardInfo>(cardInfo);
			response.Result = sharedCardInfo;
			return response;
		}
		...
		public Response GetStatus(long customerSessionId, string confirmationNumber, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = bizMoneyTransferEngine.GetStatus(customerSessionId, confirmationNumber, context);
			return response;
		}
		...
		public Response SendMoneySearch(long customerSessionId, SendMoneySearchRequest request, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.SendMoneySearch(customerSessionId, request, context); 
			return response;
		}

		[Transaction()]
		public Response StageModifySendMoney(long customerSessionId, SharedData.ModifySendMoneyRequest moneyTransferModify, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			...
			#endregion
			response.Result = SharedEngine.StageModifySendMoney(customerSessionId, moneyTransferModify, context);
			return response;
		}

		[Transaction()]
		public Response AuthorizeModifySendMoney(long customerSessionId, SharedData.ModifySendMoneyRequest modifySendMoneyRequest, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.AuthorizeModifySendMoney(customerSessionId, modifySendMoneyRequest, context);
			return response;
		}
		...
		[Transaction(ReadOnly = true)]
		public Response GetRefundReasons(long customerSessionId, SharedData.ReasonRequest request, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetRefundReasons(customerSessionId, request, context);
			return response;
		}


		[Transaction()]
		public Response StageRefundSendMoney(long customerSessionId, SharedData.RefundSendMoneyRequest moneyTransferRefund, MGIContext mgiContext)
		{
			Response response = new Response();
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.StageRefundSendMoney(customerSessionId, moneyTransferRefund, context); 
			return response;
		}
		...
		public Response SendMoneyRefund(long customerSessionId, RefundSendMoneyRequest moneyTransferRefund, MGIContext mgiContext)
		{
			...
			if (response.RefundStatus.ToUpper() == RefundStatus.F.ToString() || response.RefundStatus.ToUpper() == RefundStatus.N.ToString())
			{
				...
				#endregion
				responseResult.Result = bizMoneyTransferEngine.Refund(customerSessionId, bizRefundSendMoney, context);
			...
			responseResult.Result = string.Empty;
			return responseResult;
		}
		...
		public Response GetXfrCountries(long customerSessionId, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrCountries(customerSessionId, context);
			return response;
		...
		public Response GetXfrStates(long customerSessionId, string countryCode, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrStates(customerSessionId, countryCode, context);
			return response;
		...
		public Response GetXfrCities(long customerSessionId, string stateCode, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetXfrCities(customerSessionId, stateCode, context); 
			return response;
		...
		public Response GetCurrencyCodeList(long customerSessionId, string countryCode, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = SharedEngine.GetCurrencyCodeList(customerSessionId, countryCode, context);
			return response;
		...
		public Response GetCurrencyCode(long customerSessionId, string countryCode, MGIContext mgiContext)
		{
			...
			mgiContext = _GetContext(customerSessionId, mgiContext);
			MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = bizMoneyTransferEngine.GetCurrencyCode(customerSessionId, countryCode, context);
			return response;
		}
		...
		public Response WUGetAgentBannerMessage(long agentSessionId, MGIContext mgiContext)
		{
            ...
            mgiContext = _GetContextFromAgentSession(agentSessionId, mgiContext);
            MGI.Common.Util.MGIContext context = Mapper.Map<MGI.Common.Util.MGIContext>(mgiContext);
			response.Result = bizMoneyTransferEngine.GetBannerMsgs(agentSessionId, context);
            return response;
		}
