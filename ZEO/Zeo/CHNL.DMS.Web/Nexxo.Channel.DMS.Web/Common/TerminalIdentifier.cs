        ...
        public static void IdentifyTerminal(long agentSessionId, ZeoClient.ZeoContext context)
        {
            ...
            else if (channelPartner.TIM == (short)TerminalIdentificationMechanism.HostName)
            {
                if (HttpContext.Current.Session["HostName"] != null)
                {
                    ...
                    //Condition to throw an error - "Terminal not set" in case the location is "Inactive" for the terminal and For tech role it is not mandatory to
                    //set up the terminal as PS will not be installed.
                    if (!isterminalNotSetup && Convert.ToInt32((HttpContext.Current.Session["UserRoleId"])) != (int)TCF.Zeo.Common.Util.Helper.UserRoles.Tech)
                    {
                        ZeoClient.Response termialResponse = alloyServiceClient.GetTerminalByName(Convert.ToString(HttpContext.Current.Session["HostName"]), context);
                        if (WebHelper.VerifyException(termialResponse)) throw new ZeoWebException(termialResponse.Error.Details);
                        ZeoClient.Terminal terminal = termialResponse.Result as ZeoClient.Terminal;

                        if(!terminal.IsLocationActive)
                        {
                            HttpContext.Current.Session["IsTerminalSetup"] = 0;
                ...
                {
                    if (Convert.ToInt32((HttpContext.Current.Session["UserRoleId"])) != (int)TCF.Zeo.Common.Util.Helper.UserRoles.Tech)
                    {
                        throw new ZeoWebException(GetErrorMessage("1000.100.8091"));
                    }
                }
