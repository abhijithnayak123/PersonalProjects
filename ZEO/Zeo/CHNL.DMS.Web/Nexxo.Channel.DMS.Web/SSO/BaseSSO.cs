...
namespace TCF.Channel.Zeo.Web.SSO
{
    public abstract class BaseSSO
    {
        ...
        public abstract ZeoClient.AgentSession AuthorizeSSOAgent(string SAMLResponse, string issuer, SSOLogin ssoModel, out ZeoClient.AgentSSO ssoAgent);
        ...
        /// <returns></returns>
        protected virtual SSOErrorCodes ReadAttributes(XmlDocument assertion, out ZeoClient.AgentSSO ssoAgent)
        {
            // todo: ReadAttributes
            ssoAgent = null;
        ...
        protected virtual ZeoClient.AgentSession CreateAndAuthenticateAgent(ZeoClient.AgentSSO ssoAgent, string channelPartnerId, string terminalName, ZeoClient.ZeoContext context)
        {
            ...
            response = serviceClient.AuthenticateSSO(ssoAgent, channelPartnerId, terminalName, context);
            if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
            ZeoClient.AgentSession agentSession = response.Result as ZeoClient.AgentSession;
            return agentSession;
        }
        ...
        protected string GetAttributeNode(XmlNode samlAttributes, string attributeName)
        {
            foreach (XmlNode node in samlAttributes.ChildNodes)
            {
                ...
                if (string.Equals(att.Value, attributeName, StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    NLogHelper.Info(string.Format(attributeName + " value : " + attributeValue));
        ...
        protected string GetTerminalName(long agentSessionId, ZeoClient.ChannelPartner channelPartner, string hostName)
        {
            ...
            {
                ZeoClient.Response response = alloyClient.GetTerminalByName(hostName, context);
                if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
                var terminal = response.Result as ZeoClient.Terminal;

                if (!string.IsNullOrEmpty(hostName) && terminal != null)
        ...
        protected bool IsPeripheralServerSetUp(long agentSessionId, string terminalName, long channelPartnerId)
        {
            ...
            ZeoClient.Response alloyResponse = alloyClient.GetTerminalByName(terminalName, context);
            ...
            if (terminal != null && terminal.PeripheralServerUrl != null)
            {
        ...
        private X509Certificate2 GetSSOCertificate(long channelpartnerId, string issuer)
        {
            ...
            response = serviceClient.GetChannelPartnerCertificateInfo(channelpartnerId, issuer, context);
            if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
            ZeoClient.ChannelPartnerCertificate channelPartnerCertificate = response.Result as ZeoClient.ChannelPartnerCertificate;
