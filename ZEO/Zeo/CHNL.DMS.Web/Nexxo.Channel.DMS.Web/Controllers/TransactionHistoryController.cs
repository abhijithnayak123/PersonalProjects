    ...
    public class TransactionHistoryController : BaseController
    {
        ...
        public ActionResult GetAgentTransHistory(int page = 1, int rows = 5)
        {
            ...
            try
            {
                ...
                AgentTransactionHistory history = Session["AgentTrxHstryCriteria"] as AgentTransactionHistory;

                int currentAgentId = Session["agentId"] == null ? 0 : Convert.ToInt32(Session["agentId"]);
                bool showAllReport = history.IsTransactionStatusSelected;
                ...
                long? agentId = history.Agent == null || history.Agent == "Select" ? currentAgentId : Convert.ToInt64(history.Agent); 
                
                long transactionId = history.TransactionID == null ? 0 : (long)history.TransactionID; 

                string location = null;
                if (!string.IsNullOrWhiteSpace(Session["CurrentLocation"]?.ToString()))
                    location = Session["CurrentLocation"].ToString();
                ...
                {
                    agentId = history.Agent == null ? null : agentId;  
                }

                string transactionType = history.TransactionType == null ? null : history.TransactionType;
                ...
                ZeoClient.TransactionHistorySearchCriteria criteria = new ZeoClient.TransactionHistorySearchCriteria()
                {
                    ...
                    LocationName = location,
                    StartDate = Convert.ToDateTime(history.StartDate),
                    EndDate = Convert.ToDateTime(history.EndDate),
                ...
                TempData["AgentTransactionHistory"] = transactions;
        ...
        public ActionResult GetAgentTransactionHistory(bool IsException = false, string ExceptionMessage = "")
        {
            try
            {
                ...
                agentTranHistory.StartDate = DateTime.Now.AddDays(-duration).ToString("MM/dd/yyyy");
                agentTranHistory.EndDate = DateTime.Now.ToString("MM/dd/yyyy");

                Session["AgentTrxHstryCriteria"] = agentTranHistory;
        ...
        public ActionResult GetAgentTransactionHistory(AgentTransactionHistory agentTransactionHistory)
        {
            try
            {
                ...
                Session["AgentTrxHstryCriteria"] = agentTransactionHistory;
                Session["PdfContent"] = null;
                AgentTransactionHistory agentTranHistory = new AgentTransactionHistory();
        ...
        public ActionResult AgentSummaryPDFPreviewPopUp()
        {
            return PartialView("_partialAgentSummaryPDFPreview");
        }

        [SkipNoDirectAccess]
        public FileResult AgentSummaryPDFPreview()
        {
            try
            {
                return File(CreatePDF(), "application/pdf");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public JsonResult CreatePDFFile()
        {
            CreatePDF();
        ...
        private byte[] CreatePDF()
        {
            ...
            if (Session["PdfContent"] != null)
            {
                fileContents = Session["PdfContent"] as byte[];
            }
            else
            {
                Document document = new Document();
                document.Info.Title = "Agent Summary Report";
                document.Info.Subject = "Agent Summary Report";
                document.Info.Author = "TCF Bank";
                document.DefaultPageSetup.LeftMargin = 33;

                Table table = new Table();

                DefineStyles(document);

                CreatePage(document, ref table);

                FillContent(document, table);
                ...
                Session["PdfContent"] = fileContents;
            }
        ...
        private void FillContent(Document document, Table table)
        {
            List<ZeoClient.TransactionHistory> collection = TempData["AgentTransactionHistory"] as List<ZeoClient.TransactionHistory>;
            TempData.Keep("AgentTransactionHistory");

            for (int i = 0; i < collection.Count; i++)
            {
                Row row = table.AddRow();
                row.Cells[0].AddParagraph(collection[i].TransactionDate.ToString("MM/dd/yyyy"));
                row.Cells[1].AddParagraph(collection[i].Teller);
                row.Cells[2].AddParagraph(Convert.ToString(collection[i].SessionID));
                row.Cells[3].AddParagraph(Convert.ToString(collection[i].TransactionId));
                row.Cells[4].AddParagraph(collection[i].CustomerName);
                row.Cells[5].AddParagraph(collection[i].TransactionStatus);
                AddTextToCell(document, collection[i].TransactionType, row.Cells[6]);
                AddTextToCell(document, collection[i].TransactionDetail, row.Cells[7]);
                row.Cells[8].AddParagraph(collection[i].TotalAmount.ToString("C2"));
            }
            table.SetEdge(0, 1, 9, collection.Count, Edge.Box, BorderStyle.Single, 0.75, new Color(143, 178, 183));
        }

        private void CreatePage(Document document, ref Table table)
        {
            ...
            table.Style = "Table";
            table.Borders.Color = new Color(143, 178, 183);
            ...
            row.Shading.Color = new Color();
            row.Format.Font.Color = new Color(0, 0, 0);
            row.Cells[0].AddParagraph("Transaction Date");
            row.Cells[0].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[1].AddParagraph("Teller Name");
            row.Cells[1].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[2].AddParagraph("Session Id");
            row.Cells[2].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[3].AddParagraph("Transaction #");
            row.Cells[3].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[4].AddParagraph("Customer Name");
            row.Cells[4].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[6].AddParagraph("Transaction Type");
            row.Cells[6].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[5].AddParagraph("Transaction Status");
            row.Cells[5].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[7].AddParagraph("Transaction Details");
            row.Cells[7].Format.Alignment = ParagraphAlignment.Left;
            row.Cells[8].AddParagraph("Total Amount($)");
            row.Cells[8].Format.Alignment = ParagraphAlignment.Left;

            table.SetEdge(0, 0, 9, 1, Edge.Box, BorderStyle.Single, 0.75, new Color(143, 178, 183));
        }
        ...
        private void DefineStyles(Document document)
        {
            ...
            style.Font.Name = "Arial, sans-serif, Helvetica";
        ...
        #region This code is for to wrap word in table column
        private void AddTextToCell(Document document, string instring, Cell cell)
        {
            cell.Column.RightPadding = 2;
            TextMeasurement textMeasurement = new TextMeasurement(document.Styles["Table"].Font.Clone());
            instring = AdjustIfTooWideToFitIn(textMeasurement, cell, instring);
            cell.AddParagraph(instring);
        }

        private static string AdjustIfTooWideToFitIn(TextMeasurement textMeasurement, Cell cell, string text)
        {
            Unit availableWidth = cell.Column.Width - (cell.Column.LeftPadding + cell.Column.RightPadding);

            var tooWideWords = text.Split(" ".ToCharArray()).Distinct().Where(s => TooWide(textMeasurement, s, availableWidth));

            var adjusted = new StringBuilder(text);
            foreach (string word in tooWideWords)
            {
                var replacementWord = MakeFit(textMeasurement, word, availableWidth);
        ...
        private static bool TooWide(TextMeasurement textMeasurement, string word, Unit width)
        {
            float f = textMeasurement.MeasureString(word, UnitType.Point).Width;
        ...
        private static string MakeFit(TextMeasurement textMeasurement, string word, Unit width)
        {
            var adjustedWord = new StringBuilder();
            var current = string.Empty;
            foreach (char c in word)
            {
                if (TooWide(textMeasurement, current + c, width))
                {
