    ...
    public class TransactionSummaryController : BaseController
    {
        ...
        public ActionResult DisplayDetails(string id, string status)
        {
            ...
            try
            {
                ...
                var checkTrx = cart.Checks.Where(c => c.Id == long.Parse(id)).FirstOrDefault();
                if (cart.Checks.Count(c => c.Id == long.Parse(id)) > 0)
                {
                    try
                    {
                        // Display Check Details in pop-up    
                        context = checkTrx.ProductProviderCode == ZeoClient.HelperProviderId.Ingo ? GetCheckLogin() : context;                      
                        ...
                        checkTransaction.AmountCredited = Math.Round(checkStatus?.Amount ?? 0, 2);
                        checkTransaction.CheckCashingFee = Math.Round(checkStatus?.BaseFee ?? 0, 2);
                        //AL-657 - 5.x-Regression-For failed check, Net to Customer should be Zero.
                        checkTransaction.NetAmount = Math.Round((status == WebCommon.TransactionStatus.Failed.ToString("D")) ? (checkTransaction.AmountCredited - checkTransaction.CheckCashingFee) : 0, 2);
