    ...
    public class ShoppingCartController : BaseController
    {
        ...
        public ActionResult GetShoppingCartSummary(string sidx, string sord, int page = 1, int rows = 5)
        {
            ...
            try
            {
                ...
                if (shoppingCart.Checks != null)
                {
                    ...
                    if (check != null && check.ProductProviderCode == ZeoClient.HelperProviderId.Ingo)
                    {
        ...
        public ActionResult ShoppingCartCheckout(bool IsException = false, string ExceptionMessage = "")
        {
            ...
            try
            {
                ...
                //For onus if Discount Not Applied and if the response has text - "Commerical Exempt".
                if (shoppingCartDetail.Items.Any(i => i.DiscountApplied >= 0 && TempData["Message"] != null &&
                    TempData["Message"].ToString().ToLower().Contains("commercial exempt") && context.ProviderId == (int)Helper.ProviderId.TCFCheck))
                {
                    ViewBag.PromotionText = "* Commercial Exempt Account";
                }
        ...
        public ActionResult ShoppingCartCheckout(ShoppingCartDetail shoppingCartDetail, string submit, string recalc)
        {
            ...
            try
            {
                ...
                if (ModelState.IsValid)
                {
                    ...
                    if (shoppingCart.Checks.Any(c => c.ProductProviderCode == ZeoClient.HelperProviderId.Ingo))
        ...
        public JsonResult ParkAndDeleteShoppingCartItems(long custSessionId = 0)
        {
            try
            {
                ZeoClient.ZeoServiceClient alloyServiceClient = new ZeoClient.ZeoServiceClient();
                ...
                response = alloyServiceClient.GetShoppingCart(customerSessionId, context);
                if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
                ...
                {
                    foreach (var check in shoppingCart.Checks)
                    {
                        try
                        {
                            if (check.Status != TransactionStatus.Committed.ToString("D"))
                            {
                                context.ProviderId = (int)check.ProductProviderCode;
                                ShoppingCartHelper.ParkShoppingCartTrx(check.Id, ProductType.Checks, context, customerSessionId);
                            }
                        }
                        catch { }
                    }
                    ...
                    foreach (var item in shoppingCart.MoneyOrders)
                    {
                        try
                        {
                            if (Convert.ToString(item.State) != TransactionStatus.Committed.ToString("D"))
                                ShoppingCartHelper.ParkShoppingCartTrx(item.Id, ProductType.MoneyOrder, context, customerSessionId);
                        }
                        catch { }
                    }
        ...
        // TCF - Post Flush
        public ActionResult FinalFlushShoppingCart()
        {
            try
            {
                bool IsFinalCallPostFlush = Convert.ToBoolean(ConfigurationManager.AppSettings["IsFinalCallPostFlush"]);
                ZeoClient.ZeoServiceClient alloyServiceClient = new ZeoClient.ZeoServiceClient();
                ZeoClient.ZeoContext context = GetZeoContext();
                ZeoClient.Response response = new ZeoClient.Response();
                ...
                try
                {
                    ...
                    {
                        response = alloyServiceClient.FinalCommit(context);
                        if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
                    }
                }
                catch (Exception ex)
        ...
        public ActionResult SwipeToWithdraw()
        {
            TempData["SwipedCardNumber"] = null;

            return PartialView("_SwipeToWithdraw");
        }
