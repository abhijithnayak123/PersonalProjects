    ...
    public class CheckCashingPrgController : BaseController
    {
        ...
        public ActionResult CheckStatusProcessor(CheckCashingProgress checkCashProgress)
        {
            try
            {
                ...
                if (checkCashProgress.PromotionCode?.ToLower() == "other")
                    checkSubmit.PromoCode = checkCashProgress.ManualPromocode?.ToUpper();
                ...
                checkSubmit.ProductProviderCode = checkCashProgress.ProductProviderCode;
                ZeoClient.Check checkStatus = null;
                if (checkCashProgress.ProductProviderCode == ZeoClient.HelperProviderId.Ingo)
                    context = GetCheckLogin();
                context.ProviderId = (int)checkCashProgress.ProductProviderCode;
                context.MaxAmountOnUsCheck = Convert.ToDecimal(System.Configuration.ConfigurationManager.AppSettings["MaxCheckAmountOnUs"].ToString());
                ...
                if (checkStatus.ValidatedType != null && checkStatus.SelectedType != checkStatus.ValidatedType)
                {
                    ...
                    checkReClassified.LCheckTypes = GetCheckTypes(getProviderId(checkCashProgress.ProductProviderCode, checkCashProgress.CheckAmount));
                    ...
                    checkReClassified.ProductProviderCode = getProviderId(checkCashProgress.ProductProviderCode, checkCashProgress.CheckAmount);

                    return View("CheckReClassified", checkReClassified);
                }
                else if (checkStatus.Status.ToLower() == "approved")
                {
                    TempData["Message"] = checkStatus.DmsStatusMessage; //For OnUS to Check whether the message contains the text "Commercial Exempt
                ...
                else if (checkStatus.Status.ToLower() == "declined")
                {
                    ...
                    checkDeclined.LCheckTypes = GetCheckTypes(getProviderId(checkCashProgress.ProductProviderCode, checkCashProgress.CheckAmount));
