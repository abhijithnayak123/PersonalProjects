...
namespace TCF.Channel.Zeo.Web.Controllers
{
    public class ProductController : BaseController
    {
        ...
        public ActionResult GetFeatures(bool IsException = false, string ExceptionMessage = "")
        {
            try
            {
                ...
                List<Feature> featureDetails = features.Select(f => new Feature()
                {
                ...
                featureModel.Features = featureDetails;

                return View("FeatureList", "_Menu", featureModel);
            }
            catch (Exception ex)
            {
                VerifyException(ex);
        ...
        [HttpPost]
        public ActionResult SaveFeatureStatus(FeatureModel featureModel)
        {
            try
            {
                ZeoClient.ZeoServiceClient alloyServiceClient = new ZeoClient.ZeoServiceClient();
                ...
                if(featureModel.Features != null)
                {
                    List<ZeoClient.FeatureDetails> featureDetails = featureModel.Features.Select(f => new ZeoClient.FeatureDetails()
                    {
                        FeatureId = f.FeatureId,
                        FeatureName = f.FeatureName,
                        IsEnabled = f.IsEnabled
                    ...
                    ZeoClient.Response response = alloyServiceClient.UpdateFeatures(featureDetails, context);
                    if (WebHelper.VerifyException(response)) throw new ZeoWebException(response.Error.Details);
                }
