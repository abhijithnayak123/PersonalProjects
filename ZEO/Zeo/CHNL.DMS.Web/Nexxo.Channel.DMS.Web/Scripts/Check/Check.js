	...
	$.ajax({
		...
		success: function (data) {
			if (data.ErrorNo != undefined) {
				...
				EnableButtons();
				hideSpinner();
				getMessage(EpsonException.Check_Scan_Error, data.ErrorDetails);
				return;
			}
			...
			ExtractValues(data.ScanCheckResult.Micr);
		...
		error: function (data, errmessage, errdetails) {
			...
			if (data.ErrNo != undefined) {
				...
				getMessage(EpsonException.Check_Scan_Error);
				return;
			}
		},
					...
					for (var receiptIndex = 0; receiptIndex < printParams.length; receiptIndex++) {
						for (var copyIndex = 0; copyIndex < printParams[receiptIndex].NumberOfCopies; copyIndex++) {
							if (printParams[receiptIndex].PrintData != "")
								...
								hideSpinner();
								getMessage(EpsonException.Check_Error_declined);//change made by shwetha
							}
						}
					}
				} else {
...
function ExtractValues(micr) {
    var routingNumber = micr.match(/t(.+)t/)[1].trim();
    var accountNumber = micr.match(/t(.+)o/);
    var checkNumber = "0";
    if (accountNumber != null && accountNumber.length >= 2 && accountNumber[1] != null) {
        accountNumber = accountNumber[1].toString().split("t")[1].replace("o", "").trim();
        if (accountNumber.indexOf("o") != -1) {
            accountNumber = accountNumber.split("o")[0];
            checkNumber = accountNumber.split("o")[1];
        } else {
            var plainMicr = micr.replace(/t/g, "").replace(/o/g, "");
            checkNumber = plainMicr.replace(routingNumber, "").replace(accountNumber, "").trim();
        }
    }
