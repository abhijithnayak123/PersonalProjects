...
jQuery.validator.unobtrusive.adapters.add
    ...
    jQuery.validator.unobtrusive.adapters.add
        ("sqlMinDateValidation", ['minvalue'],
        function (options) {
            options.rules['sqlMinDateValidation'] = {
                other: options.params.other,
                minvalue: options.params.minvalue,
            };
            options.messages['sqlMinDateValidation'] = options.message;
        }
    );


    jQuery.validator.addMethod("sqlMinDateValidation", function (value, element, params) {
        if (value == "" || value == '__/__/____' || value == 'MM/DD/YYYY') {
            return true;
        }
        if ($(element)) {
            ...
            var sqldateMin = new Array();

            sqldateMin[0] = params.minvalue.split('/')[0]; //month
            sqldateMin[1] = params.minvalue.split('/')[1]; //date
            sqldateMin[2] = params.minvalue.split('/')[2]; //year

            var sqlMinDate = new Date();
            sqlMinDate.setMonth(sqldateMin[0] - 1);
            sqlMinDate.setDate(sqldateMin[1]);
            sqlMinDate.setFullYear(sqldateMin[2]);
            if (sqlMinDate <= enteredDate) {
                return true;
            }
            return false;
        }
...
jQuery.validator.addMethod("myconditionalrequiredcustomvalidation", function (value, element, params) {
	if (params.propertytwo == 'IssueDateCondition') {
		if ($('#' + params.propertyone)) {
			...
			if (propertyonevalue.toLowerCase() == 'driver\'s license' || propertyonevalue.toLowerCase() == 'employment authorization card (ead)' || propertyonevalue.toLowerCase() == 'permanent resident card') {
				if (value == null || value == "")
					return false;
				else
					return true;
...
$.validator.addMethod('greaterthanmin', function (value, element, param) {
    var dependingvalue = $('#' + param.dependingproperty).val();
    var validateminvalue = param.validateminvalue;
    var valid = false;

    if (value == "" || dependingvalue == "")
        return true;

    if ($(element)) {

        var currentValue = parseInt(value, 10); 
        var targetedValue = parseInt(dependingvalue, 10); 

        if (validateminvalue === "true")
        {
