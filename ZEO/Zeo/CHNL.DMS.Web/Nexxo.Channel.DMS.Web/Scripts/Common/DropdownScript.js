$(document).ready(function () {
    ...
    $('#MotherMaidenName').focus();//To hide the clearlink('X') on load

	//$('#ClientID').attr('disabled', true);

    $('#LegalCode').change(function () {
        var selectedProvinceId = $("#Country").val();
        var legalCode = $("#LegalCode").val();
        var idType = $("#GovtIDType").val();
        getIdType(selectedProvinceId, legalCode, idType);
    });
    ...
    $('#Country').change(function () {
		...
		var legalCode = $("#LegalCode").val();
		stateValidation();
		getIdType(selectedProvinceId, legalCode, '');
	});
	$('#GovtIDType').change(function () {
		...
		$.ajax({
			...
			success: function (data) {
				if (handleException(data)) return;
...
function SetMandatoryForIdType() {
	...
	if (selectedGovtTypeId == 'driver\'s license' || selectedGovtTypeId == 'new york city id' || selectedGovtTypeId == 'employment authorization card (ead)' || selectedGovtTypeId == 'permanent resident card')
	...
	if (selectedGovtTypeId == 'employment authorization card (ead)' || selectedGovtTypeId == 'permanent resident card' || selectedGovtTypeId == 'passport' || selectedGovtTypeId == 'military id' || selectedGovtTypeId == 'matricula consular' || selectedGovtTypeId == 'instituto federal electoral' || selectedGovtTypeId == "")
		$('#stateRequired').hide();
	else
		$('#stateRequired').show();
}

function getIdType(selectedProvinceId, legalCode, idType) {
    $.ajax({
        url: base_url_GovtIdType,
        data: { CountryId: selectedProvinceId, legalCode: legalCode }, //parameters go here in object literal form 
        type: 'GET',
        datatype: 'json',
