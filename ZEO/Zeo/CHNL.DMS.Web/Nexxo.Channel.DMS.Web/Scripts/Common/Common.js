...
$(document).ready(function () {
    ...
    $('#DomesticPreDisclosure').click(function () {
        disableNextButton();
    });
    reOpenChatDialog();
    ...
    if (sessionStorage["minimize-helpbox"]) {
        var value = sessionStorage.getItem("minimize-helpbox");
        if (value.toLowerCase() === "true") {
            minimHelpbox();
        }
    }

    //For Zeo reference manual pop up
    checkReferenceManualPopup();
});
...
(event.keyCode >= 35 && event.keyCode <= 39)) {
        ...
        //If the card number is more then 16 digit, if the teller press enter key- call focusout method to mask the card number and validate the card number with voltage.
        else if (cardNumberCount >= 16 && event.keyCode == 13) {
...
function parkAndDeleteShoppingCartItemsWithoutRedirect(customerSessionId) {
    $.ajax({
        url: shoppingCartItemsUrl,
        data: { custSessionId: customerSessionId },
        async: false,
        type: 'GET',
        datatype: 'json',
        success: function (jsonData) {
...
function onHelpClick() {
    $("#help-window").addClass("show-pdf-pop-onhelp").removeClass("show-pdf-pop-on-minimize");
    $("#help-window").show(); 
    $("#minim-help-window").show();
    $("#maxi-help-window").hide();
    sessionStorage.setItem("minimize-helpbox", false);
    if (loadWhatsNewInZeo) {
        showSpinner();
        var object = "<object id=\"objectID\" data=" + WhatsNewZeo_URL + " type=\"application/pdf\" width=\"100%\" height=\"100%\" standby=\"Loading....\" title=\"Loading..\">";
        object += "</object>";
        $("#whatsNewPdf").html(object);
        hideSpinner();
        loadWhatsNewInZeo = false;
    }
...
function closeHelpbox() {
    ...
    sessionStorage.setItem("minimize-helpbox", false);
    checkReferenceManualPopup();
    loadWhatsNewInZeo = true;
}

function minimHelpbox() {
    $("#minim-help-window").hide();
    $("#maxi-help-window").show();
    $("#help-window").show();
    $("#help-window").addClass("show-pdf-pop-on-minimize").removeClass("show-pdf-pop-onhelp");
    sessionStorage.setItem("minimize-helpbox", true);
    checkReferenceManualPopup();
}

//For Zeo reference manual
function onZeoManualClick() {
    $("#zeoreferencemanual").addClass("show-pdf-referencemanual").removeClass("show-referencemanualpdf-pop-on-minimize");
    setRightCSSReferenceManualPopUp('show-pdf-referencemanual', '');
    $("#zeoreferencemanual").show();
    $("#minim-manualreference-window").show();
    $("#maxi-manualreference-window").hide();
    sessionStorage.setItem("minimize-referencebox", false);
    if (loadZEORefManual) {
        showSpinner();
        var object = "<object id=\"objectID\" data=" + ZeoManualRef_URL + " type=\"application/pdf\" width=\"100%\" height=\"100%\" standby=\"Loading....\" title=\"Loading..\">";
        object += "</object>";
        $("#ZeoRefManualId").html(object);
        hideSpinner();
        loadZEORefManual = false;
    }
}

function closeReferencebox() {
    $("#zeoreferencemanual").hide();
    sessionStorage.setItem("minimize-referencebox", false);
    loadWhatsNewInZeo = true;
}

function minimReferencebox() {
    $("#minim-manualreference-window").hide();
    $("#maxi-manualreference-window").show();
    $("#zeoreferencemanual").show();
    $("#zeoreferencemanual").addClass("show-referencemanualpdf-pop-on-minimize").removeClass("show-pdf-referencemanual");
    sessionStorage.setItem("minimize-referencebox", true);
    checkHelpPdf();
}

function onContactClick() {
    ShowPUPWithDynId(ContactInfoURL, "Contact Details", 700, 200, "contactdetails");
    $('#contactdetails').parent().attr("id", "custom_Title");
}

function checkReferenceManualPopup() {
    if (sessionStorage["minimize-referencebox"]) {
        var manualValue = sessionStorage.getItem("minimize-referencebox");
        if (manualValue.toLowerCase() === "true") {
            minimReferencebox();
            checkHelpPdf();
        }
    }
...
function wordCounter(element, wordCounterId) {
    setTimeout(function () {
        ...
        var left = max - cur;
        $('#' + wordCounterId).text(left);

    }, 0);
}

function setRightCSSReferenceManualPopUp(styleClass, rightCss) {
    $('.' + styleClass).css('right', rightCss == undefined ? "" : rightCss);
}

function checkHelpPdf() {
    if (sessionStorage["minimize-helpbox"]) {
        var helpValue = sessionStorage.getItem("minimize-helpbox");
        if (helpValue.toLowerCase() === "true") {
            setRightCSSReferenceManualPopUp('show-referencemanualpdf-pop-on-minimize', '210px');
        }
        else {
            setRightCSSReferenceManualPopUp('show-referencemanualpdf-pop-on-minimize', '');
        }
    }
}

//Checking whether the Promotion meets all the qualifier if it is registered in ZEO or not.
function validatePromocode(type, amount, promoCode, promocodeid, providerId)
{
    $.ajax({
        url: ValidatePromoCode_URL,
        data: { type: type, amount: amount, promoCode: promoCode, providerId: providerId },
        type: 'GET',
        datatype: 'json',
        success: function (jsonData) {
            if (jsonData.data == 0 && jsonData.isPromoCodeEmpty === false) {
                $('#PromotionId').val(0);
                $('#promodiv span').text(jsonData.errorMessage);
                $('#' + promocodeid).val('');
            }
            else if (jsonData.data > 1) {
                $('#PromotionId').val(jsonData.data);
                $('#promodiv span').text('');
            }
            else if (jsonData.data === 1 || jsonData.isPromoCodeEmpty === true) {
                $('#PromotionId').val(0);
                $('#promodiv span').text('');
            }
        },
...
function addCrossButton() {
    $("input.input_box").addClass("clearable");
    $("input.disable_txt").removeClass("clearable");
    $("input.row_input_bg").removeClass("clearable");
}
