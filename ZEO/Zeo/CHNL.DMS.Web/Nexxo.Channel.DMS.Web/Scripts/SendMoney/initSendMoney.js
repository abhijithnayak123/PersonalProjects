...
$(document).ready(function () {
    ...
    $("#continueButton").click(function (event) {
        ...
        var actOnMyBehalf = $('#ActOnMyBehalf').val();
        if (actOnMyBehalf == "0") {
    		getMessage(MoneyTransferException.MoneyTransfer_ActOnMyBehalf);
            event.preventDefault();
        }

        //WU Fraud Changes - Starts Here
        var isFormVaild = $("#sendMoneyForm").valid();
    ...
    if (isExceptionOccured.toLocaleLowerCase() != 'true') {
        showBSAWarning();
    }
});
...
$(document).ready(function () {
    ...
    $('#CouponPromoCode, #Amount').on('change', function () {
    ...
    if (selectedCountryCode === 'CN') {
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.isShowBSAWarning) {
                    hideSpinner();
                    ShowPopUpMinHeight(showBSAWarningURL, 'Message', 373, 150);
                }
...
//function fraudWarningConsent(consumerFraudPromptQuestion) {
//    isFormPost = true;
//    hideSpinner();
