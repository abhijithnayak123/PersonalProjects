...
namespace MGI.Biz.MoneyTransfer.Impl
{
    ...
    //                                      "Begin GetCountries-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
				...
				List<string> topCountries = new List<string>() { "UNITED STATES", "CANADA", "MEXICO" };
				var orderByCountryName = countries.OrderByDescending(c => topCountries.Contains(c.Name.ToUpper())).ThenBy(c => c.Name).ToList();
				var topItem = orderByCountryName.Single(x => x.Name.ToUpper() == "UNITED STATES");
				List<bizMasterData> masterCountryList = new List<bizMasterData>();
				masterCountryList.Add(topItem);
				masterCountryList = masterCountryList.Concat(orderByCountryName.Where(c => c.Name.ToUpper() != "UNITED STATES")).ToList();
				//#region AL-3370 Transactional Log User Story
    ...
    //                                      "End GetCountries-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetCountries", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetCountries-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETCOUNTRIES_FAILED, ex);
			}
		}
      ...
      //                                    "Begin GetStates-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
			   //#endregion
				IMoneyTransfer CXNMoneyTransfer = _GetMoneyTransferProcessor(mgiContext.ChannelPartnerName);
				List<cxnMasterData> cxnStates = CXNMoneyTransfer.GetStates(countryCode).OrderBy(a => a.Name).ToList();
				List<bizMasterData> states = Mapper.Map<List<cxnMasterData>, List<bizMasterData>>(cxnStates);
      ...
      //                                    "End GetStates-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetStates", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetStates-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETSTATES_FAILED, ex);
			}
		}
      ...
      //                                    "End GetCities-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
            ...
            {
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetCities", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetCities-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETCITIES_FAILED, ex);
            }
		}

        public string GetBannerMsgs(long agentSessionId, MGIContext mgiContext)
		{
			try
			{
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(agentSessionId), "GetBannerMsgs", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetBannerMsgs-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETBANNERMSGS_FAILED, ex);
			}
		}
        ...
        public string GetCurrencyCode(long customerSessionId, string countryCode, MGIContext mgiContext)
		{
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetCountries", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetCurrencyCode-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETCOUNTRIES_FAILED, ex);
			}
		}
      ...
      //                                    "End GetCurrencyCodeList-MGI.Biz.MoneyTransfer.Impl.MoneyTransferSetupEngine", mgiContext);
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetCountries", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetCurrencyCodeList-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETCURRENCYCODELIST_FAILED, ex);
			}
		}
		...
		public List<Reason> GetRefundReasons(long customerSessionId, ReasonRequest request, MGIContext mgiContext)
		{
			...
			{
				//MongoDBLogger.Error<string>(Convert.ToString(customerSessionId), "GetRefundReasons", AlloyLayerName.BIZ, ModuleName.SendMoney,
				//					  "Error in GetRefundReasons-MGI.Biz.MoneyTransfer.Impl.MoneyTransferEngine", ex.Message, ex.StackTrace);
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMoneyTransferException(BizMoneyTransferException.MONEYTRANSFER_GETREFUNDREASONS_FAILED, ex);
			}
