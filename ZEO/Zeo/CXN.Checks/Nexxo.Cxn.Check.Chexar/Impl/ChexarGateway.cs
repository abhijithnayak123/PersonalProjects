...
namespace MGI.Cxn.Check.Chexar.Impl
{
	...
	public class ChexarGateway : ICheckProcessor, IChexarPartnerConfigurator
	{
		...
		public CxnCheckData.CheckStatus Submit(long trxId, long accountId, CxnCheckData.CheckInfo check, MGIContext mgiContext)
		{
			try
			{
				string chexarLocation = generateChexarLocationId(mgiContext.ChannelPartnerId, IngoUserName(mgiContext));
				...
				chxrTrx.SubmitType = chxrTrx.ReturnType = getChexarType(check.Type);

				chxrTrx.Id = trxId;
				chxrTrx.Location = chexarLocation;
				//timestamp changes
				chxrTrx.DTTerminalCreate = MGI.TimeStamp.Clock.DateTimeWithTimeZone(mgiContext.TimeZone);
				chxrTrx.DTServerCreate = DateTime.Now;
				chxrTrx.ChexarStatus = "not submitted yet";
				// Karun@10/14/2013: added PartnerId to ChexarTrx
				chxrTrx.ChannelPartnerID = mgiContext.ChannelPartnerId;

				ChexarAccount account = _accountRepo.FindBy(x => x.Id == accountId);

				if (account == null)
					throw new CheckException(CheckException.ACCOUNT_NOT_FOUND);
				...
				chxrTrx.DTTerminalLastModified = MGI.TimeStamp.Clock.DateTimeWithTimeZone(mgiContext.TimeZone);
			...
			catch (Exception ex)
			{
				...
				throw new CheckException(CheckException.CHEXAR_SUBMIT_FAILED, ex);
			}
		}
		...
		public void Commit(long trxId, string timezone, MGIContext mgiContext)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_COMMIT_FAILED, ex);
			}
		}
		...
		public CxnCheckData.CheckTrx Get(long trxId)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_GET_TRANSACTION_FAILED, ex);
			}
		}
		...
		public CxnCheckData.CheckStatus Status(long trxId, string timezone, MGIContext mgiContext)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_GET_STATUS_FAILED, ex);
			}
		}
		...
		public bool Cancel(long trxId, string timezone, MGIContext mgiContext)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_CANCEL_TRANSACTION_FAILED, ex);
			}
		...
		public CxnCheckData.CheckProcessorInfo GetCheckProcessorInfo(string locationId)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_GET_CHECK_PROCESSOR_INFO_FAILED, ex);
			}
		}
		...
		public void UpdateTransactionFranked(long trxId)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_UPDATE_TRANSACTION_FAILED, ex);
			}
		}
		...
		public List<CxnCheckData.CheckTrx> PendingChecks()
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_PENDING_CHECK_FAILED, ex);
			}
		}
		#endregion
		...
		public long Register(MGI.Cxn.Check.Data.CheckAccount account, MGIContext mgiContext, string timezone)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_REGISTER_FAILED, ex);
			}
		}
		...
		public void Update(MGI.Cxn.Check.Data.CheckAccount account, MGIContext mgiContext)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_ACCOUNT_UPDATE_FAILED, ex);
			}
		}
		...
		public Cxn.Check.Data.CheckAccount GetAccount(long accountId)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_GET_ACCOUNT_FAILED, ex);
			}
		}
		...
		public void SetupPartner(ChexarPartner partner)
		{
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_SETUP_PARTNER_FAILED, ex);
			}
		}
		#endregion
		...
		private ChexarTrx getChexarTrx(long trxId)
		{
			...
			if (chexarTrx == null)
				throw new CheckException(CheckException.TRANSACTION_NOT_FOUND);
		...
		private string IngoUserName(MGIContext mgiContext)
		{
			if (string.IsNullOrEmpty(mgiContext.CheckUserName))
				throw new CheckException(CheckException.LOCATION_NOT_SET);
		...
		private int getChexarType(CxnCheckData.CheckType checkType)
		{
			...
			if (chexarTypes.Count == 0)
				throw new CheckException(CheckException.CHEXAR_CHECK_TYPE_NOT_FOUND);
		...
		private void cancelWithChexar(ChexarIO.ChexarLogin login, ChexarTrx chexarTrx)
		{
			...
			catch (ChexarProviderException ex)
			{
		...
		public CxnCheckData.CheckLogin GetCheckSessions(MGIContext mgiContext)
		{
			try
			{
				...
				if (channelPartner == null)
					throw new CheckException(CheckException.CHEXAR_CREDENTIALS_NOT_FOUND);
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new CheckException(CheckException.CHEXAR_GET_SESSION_FAILED, ex);
			}
		}
		#endregion
	}
