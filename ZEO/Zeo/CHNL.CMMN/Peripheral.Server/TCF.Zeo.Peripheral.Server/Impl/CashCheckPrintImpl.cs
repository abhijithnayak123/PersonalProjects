    ...
    public partial class PeripheralServiceImpl : IPrinter
    {
        public PrintResponse PrintDocStream(String printparams)
        {
            ...
            //try
            {
                ...
                printList[0] = DecodeReceiptContent(printList[0]); // Encoding and decoding the transactions data so that we will print all the unicode characters.
        ...
        private string DecodeReceiptContent(string encodedData)
        {
            try
            {
                encodedData = encodedData.Replace(" ", "+");
                string encodedTags = encodedData.Substring(0, encodedData.IndexOf("|"));
                string restOfTheValue = encodedData.Substring(encodedData.IndexOf("|"), encodedData.Length - encodedData.IndexOf("|"));
                byte[] convertedEncodedTags = Convert.FromBase64String(encodedTags);
                string decodeData = System.Text.Encoding.Unicode.GetString(convertedEncodedTags) + restOfTheValue;
                return decodeData;
            }
            catch (Exception ex)
            {
                Trace.WriteLine("Error while decoding the receipt content from Base64string " + ex.StackTrace);
                Trace.WriteLine(encodedData);
                return "";
            }
        }
    }
