...
namespace MGI.Core.CXE.Test
{
	...
	public class CheckExceptionTests
	{
		...
		public void StageCheckLookupThrows()
		{
			...
			MinorCodeMatch<CXECheckException>(() => _svc.GetStatus(1), CXECheckException.CHECK_GET_FAILED);
		}
		...
		{
			mokCheckStageRepo.Setup(m => m.FindBy(It.IsAny<Expression<Func<Data.Transactions.Stage.Check, bool>>>())).Throws(new Exception());
			MinorCodeMatch<CXECheckException>(() => _svc.Update(1, 2,null), CXECheckException.CHECK_GET_FAILED);
		}
		...
		public void CommitCheckLookupThrows()
		{
			...
			MinorCodeMatch<CXECheckException>(() => _svc.Get(1), CXECheckException.CHECK_GET_FAILED);
		}
