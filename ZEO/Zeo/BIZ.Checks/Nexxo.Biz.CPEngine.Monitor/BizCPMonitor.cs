								...
								try
								{
									...
									if (status != CXNData.CheckStatus.Pending)
									{
										...
										else if (status == CXNData.CheckStatus.Declined)
										{
											...
											Core.Partner.Data.Message _message = MessageStore.Lookup(ptnrCheck.CustomerSession.AgentSession.Terminal.ChannelPartner.Id, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." +bizcpexception.AlloyErrorCode, lang);
											if (_message != null)
											{
