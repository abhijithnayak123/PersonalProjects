	...
	public class MessageCenterServiceImpl : IMessageCenterService
	{
		...
		public bool Update(long customerSessionId, AgentMessage agentMessage, MGIContext mgiContext)
		{
			...
			catch (Exception ex)
			{
				...
				throw new BizMessageCenterException(BizMessageCenterException.AGENT_MESSAGE_UPDATE_FAILED, ex);
			}
		}
		...
		public List<AgentMessage> GetMessagesByAgentID(long agentSessionId, MGIContext mgiContext)
		{
			try
			{
				...
				foreach (var message in messages)
				{
					...
					if (Convert.ToInt32(cxnCheck.DeclineCode) != 0)
					{
						//BizCustomerException
						MGI.Biz.CPEngine.Data.BizCPEngineException bizcheckdeclineexception = new MGI.Biz.CPEngine.Data.BizCPEngineException(cxnCheck.DeclineCode);
						MGI.Core.Partner.Data.Message _message = MessageStore.Lookup(customer.ChannelPartnerId, BizCPEngineException.ProductCode + "." + ((int)ProviderId.Ingo).ToString() + "." + bizcheckdeclineexception.AlloyErrorCode, lang);
			...
			{
				if (ExceptionHelper.IsExceptionHandled(ex)) throw;
				throw new BizMessageCenterException(BizMessageCenterException.AGENT_MESSAGE_GET_FAILED, ex);
			}
		}
