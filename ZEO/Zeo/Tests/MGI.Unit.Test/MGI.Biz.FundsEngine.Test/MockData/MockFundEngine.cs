...
namespace MGI.Unit.Test.MGI.Biz.FundsEngine.Test.MockData
{
	public class MockFundEngine : IntializMoqObject
	{
		...
		public IFundProcessor CreateInstanceOfFundProcessor()
		{
			...
			ProcessorRouter.Setup(moq => moq.Authenticate(It.IsAny<string>(), It.IsAny<MGIContext>(), out processorResult)).Returns(
				(string cardNumber, MGIContext mgiContext, CXNData.ProcessorResult test) => 
				{
					long id = 1000000000;
					if (cardNumber == "4756756000179825")
					{
						processorResult.IsSuccess = true;
						id = 0;
					}
					if (cardNumber == "4756756000179826")
					{
						processorResult.IsSuccess = false;
						processorResult.Exception = new Exception();
					}
					if (cardNumber == "4756756000179827")
					{
						processorResult.IsSuccess = false;
			...
			ProcessorRouter.Setup(moq => moq.Get(It.IsAny<long>(), It.IsAny<MGIContext>())).Returns(
				(long trxnId, MGIContext mgiContext) => 
				{
					var existingFundtrxn = fundTrxns.Find(x => x.TransactionID == Convert.ToString(trxnId));
					if (existingFundtrxn == null)
					{
						existingFundtrxn = fundTrxns.FirstOrDefault();
					}
					return existingFundtrxn;
				});

			ProcessorRouter.Setup(moq => moq.GetBalance(It.IsAny<long>(), It.IsAny<MGIContext>(), out processorResult)).Returns(
				(long Id, MGIContext mgiContext, CXNData.ProcessorResult test) => 
				{
					if (mgiContext.AlloyId == 1000000001)
					{
						throw new FundException("1000");
					}
					if (mgiContext.AlloyId == 1000000002)
					{
						throw new FundException("2107");
					}
					return new CXNData.CardInfo();
				});
			...
			ProcessorRouter.Setup(moq => moq.Load(It.IsAny<long>(), It.IsAny<CXNData.FundRequest>(), It.IsAny<MGIContext>(), out processorResult)).Returns(
				(long id, CXNData.FundRequest fund, MGIContext mgiContext, CXNData.ProcessorResult test) =>
				{
					if (fund.Amount == 53)
					{
						return 0;
			...
			ProcessorRouter.Setup(moq => moq.Lookup(It.IsAny<long>())).Returns(
				(long accountId) => 
				{
					if (accountId == 1000000000000000 || accountId == 10000000000000000)
					{
						return new CXNData.CardAccount();
					}
					return cardAccounts.Find(a => a.Id == accountId);
				});
