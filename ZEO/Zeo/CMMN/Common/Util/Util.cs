...
namespace MGI.Common.Util
{
	public static class NexxoUtil
	{
		...
		public static string GetProductCode(string productName)
		{
			switch (productName)
			{
				case "sendmoney": return Convert.ToString(((int)MGI.Common.Util.ProductCode.MONEY_TRANSFER_PRODUCTCODE));
				case "billpay": return Convert.ToString((int)(MGI.Common.Util.ProductCode.BILL_PAY_PRODUCTCODE));
				default: return Convert.ToString(((int)MGI.Common.Util.ProductCode.MONEY_TRANSFER_PRODUCTCODE));
			}
		}
		...
		public static string GetProviderCode(string providerName)
		{
			switch (providerName)
			{
				case "sendmoney": return Convert.ToString((int)MGI.Common.Util.ProviderId.WesternUnion);
				case "billpay": return Convert.ToString((int)MGI.Common.Util.ProviderId.WesternUnionBillPay);
				default: return Convert.ToString((int)MGI.Common.Util.ProviderId.WesternUnion);
			}
		}
		...
		public static string GetProcessor(int ChannelPartnerId, int MajorCode)
		{
			...
			switch (MajorCode)
			{
				case (int)ProductCode.CHECK_PROCESSING_PRODUCTCODE:
					...
					break;
				case (int)ProductCode.FUNDS_PRODUCTCODE:
					switch (ChannelPartnerId)
					{
						case (int)ExceptionChannelPartners.Synovus:
							Processor = "VisaDPS";
					...
					break;
				case (int)ProductCode.BILL_PAY_PRODUCTCODE:
				case (int)ProductCode.MONEY_TRANSFER_PRODUCTCODE:
					switch (ChannelPartnerId)
					{
						case (int)ExceptionChannelPartners.Synovus:
						case (int)ExceptionChannelPartners.TCF:
					...
					break;
				case (int)ProductCode.MONEY_ORDER_PRODUCTCODE:
					switch (ChannelPartnerId)
					{
						case (int)ExceptionChannelPartners.Synovus:
					...
					break;
				case (int)ProductCode.CLIENT_CUSTOMER_EXCEPTION:
					switch (ChannelPartnerId)
					{
						case (int)ExceptionChannelPartners.Synovus:
							Processor = "FIS";
	...
	{
		CASH_EXCEPTION = 1000,
		CUSTOMER_PRODUCTCODE = 1001,
		CHECK_PROCESSING_PRODUCTCODE = 1002,
		FUNDS_PRODUCTCODE = 1003,
		BILL_PAY_PRODUCTCODE = 1004,
		MONEY_TRANSFER_PRODUCTCODE = 1005,
		MONEY_ORDER_PRODUCTCODE = 1006,
