    ...
    public class WUReceiptServiceImpl : BaseReceiptServiceImpl
    {
        ...
        public override BizData.Receipt GetDoddFrankReceipt(long transactionId, commonData.ZeoContext context)
        {
            try
            {
                ...
                BizData.Receipt receipt = GetDoddfrankReportTemplate(receiptData.ClientName, receiptData.DestinationCountryCode, receiptData.IsFixOnSend);
        ...
        private Data.Receipt GetDoddfrankReportTemplate(string partner, string destinationCountryCode, bool isFixOnSend = false)
        {
            ...
            receiptfileNames.Add(string.Format("{0}.{1}.{2}.{3}.Receipt.docx", partner, sreport, type, destinationCountryCode));
            receiptfileNames.Add(string.Format("{0}.{1}.{2}.Receipt.docx", partner, sreport, type));
        ...
        private string GetMoneyTransferTrxTags(PTNRData.MoneyTransferReceiptData receiptData)
        {
           ...
           if(receiptData.ReceiverAddress.Contains("#"))
            ...
            receiptBuilder.Append("|{TotalToReceiver}|" + (receiptData.TotalToReceiver != 0 ? receiptData.TotalToReceiver : receiptData.TransferAmount));
