...
namespace TCF.Zeo.Cxn.WU.Common.Impl
{
    public class SimulatorIO : BaseIO, IWUCommonIO
    {
        ...
        {
            string govtIdMapping = string.Empty;

            Dictionary<string, string> govtIdTypeMapping = CXNHelper.GetGovtIdTypes((int)Helper.ProviderId.WesternUnion);

            if (govtIdTypeMapping.ContainsKey(idType))
            {
                govtIdMapping = govtIdTypeMapping[idType];
            }

            return govtIdMapping;
        }
