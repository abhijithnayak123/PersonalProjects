    ...
    public class TCFCustomer : ICustomerRepository
    {
        ...
        public long RegisterToClient(commonData.ZeoContext context)
        {
            ...
            try
            {
                ...
                if (Convert.ToBoolean(System.Configuration.ConfigurationManager.AppSettings["IsCustomerRegistrationThroughIIB"]))
                    clientId = ClientCustomerService.Add(cxnCustomer, context);
                else
                    clientId = ClientCustomerService.RegisterCustomer(cxnCustomer, context);
        ...
        public List<CustomerProfile> SearchCustomers(CustomerSearchCriteria criteria, commonData.ZeoContext context)
        {
            ...
            try
            {
                ...
                else if (!string.IsNullOrWhiteSpace(criteria.CardNumber))
                {
                    customers = GetCardCustomerDetails(criteria, context); // card search either in Zeo or RCIF based on the card type
                }
                else
                {
                    customers = GetZeoCustomers(criteria, context); // only in Zeo DB with the DOB and Last name
                }

                if(customers.Count == 0)
                    throw new CustomerException(CustomerException.NO_CUSTOMERS_FOUND);
        ...
        public bool IsCardValidForCustomer(string cardnumber, commonData.ZeoContext context)
        {
            List<CustomerProfile> customers = new List<CustomerProfile>();
            ...
            try
            {
                ...
                string plainCardNumber = cardnumber;

                criteria.ChannelPartnerId = context.ChannelPartnerId;

                criteria.CardNumber = EncryptCardNumber(cardnumber);

                customers = SearchCardCustomer(criteria, context);

                if (customers?.Count > 0)
                {
                    CustomerProfile cardCustomer = customers.FirstOrDefault();

                    if (cardCustomer.CustomerId == Convert.ToString(context.CustomerId))
                    {
                        isCardValid = true;
                        return isCardValid;
                    }
                ...
                throw new CustomerException(CustomerException.CUSTOMER_ZEOCARD_DETAILS_NOT_MATCHING);
            }
            catch (Exception ex)
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
                throw new CustomerException(CustomerException.CUSTOMER_ZEOCARD_VALIDATION_FAILED, ex);
            }            
        }
        ...
        {
            if(!string.IsNullOrWhiteSpace(criteria.AccountNumber) || !string.IsNullOrWhiteSpace(criteria.SSN))
            {
                return true;
            }
            return false;
        ...
        private List<CustomerProfile> GetRcifCustomers(CustomerSearchCriteria criteria, commonData.ZeoContext context)
        {
            ...
            ClientCustomerService = GetClientCustomerService(context.ChannelPartnerName);
            ...
            if (criteria.IsAutoSearch)
                customers = GetCustomersForAutoSearch(cxnCustomerProfiles, cxnCriteria, context);
            else
                customers = GetCustomersForSingleSearch(cxnCustomerProfiles, cxnCriteria, context);            

            customers = customers.Count > 0 ? customers.Concat(cxnCustomerProfiles.Where(x => customers.Any(y => y.ClientCustomerId?.TrimStart('0') != x.ClientCustomerId?.TrimStart('0')))).ToList() : cxnCustomerProfiles;           
        ...
        /// <param name="context"></param>
        /// <returns></returns>
        private List<CustomerProfile> SearchCardCustomer(CustomerSearchCriteria criteria, commonData.ZeoContext context)
        {
            try
            {
                ProcessorRouter = new ProcessorRouter();
        ...
        private List<CXNDATA.CustomerProfile> GetCustomersForSingleSearch(List<CXNDATA.CustomerProfile> cxnCustomerProfiles, CXNDATA.CustomerSearchCriteria cxnCriteria, commonData.ZeoContext context)
        {
            if (cxnCustomerProfiles.Count > 0 || !string.IsNullOrWhiteSpace(cxnCriteria.SSN))
            {
               return GetClientCustomerService(context.ChannelPartnerName).GetCustomersForSingleSearch(cxnCustomerProfiles, cxnCriteria.SSN, context);
            }

            return new List<Cxn.Customer.Data.CustomerProfile>();
        }

        private List<CXNDATA.CustomerProfile> GetCustomersForAutoSearch(List<CXNDATA.CustomerProfile> cxnCustomerProfiles, CXNDATA.CustomerSearchCriteria cxnCriteria, commonData.ZeoContext context)
        {
            return GetClientCustomerService(context.ChannelPartnerName).GetCustomersForAutoSearch(cxnCustomerProfiles, cxnCriteria, context);
        }

        private IClientCustomerService GetClientCustomerService(string channelPartnerName)
        {
            ProcessorRouter ProcessorRouter = new ProcessorRouter();

            if(ClientCustomerService == null)
                ClientCustomerService = ProcessorRouter.GetCXNCustomerProcessor(channelPartnerName);
