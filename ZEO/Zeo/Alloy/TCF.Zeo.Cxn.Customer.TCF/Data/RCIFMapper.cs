...
namespace TCF.Zeo.Cxn.Customer.TCF.Data
{
    internal static class RCIFMapper
    {
        ...
        private static string MapToAlloyIdType(string clientIdType)
        {
            ...
            Dictionary<string, string> idMapping = CXNHelper.GetGovtIdTypes((int)ProviderId.TCISCustomer);

            if (idMapping.ContainsValue(clientIdType))
            {
                idType = Convert.ToString(idMapping.Where(x => x.Value == clientIdType).Select(c => c.Key).FirstOrDefault());
            }
            return idType;
        }
