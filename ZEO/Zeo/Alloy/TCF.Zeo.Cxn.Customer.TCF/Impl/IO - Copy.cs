...
namespace TCF.Zeo.Cxn.Customer.TCF.Impl
{
    public class IO : IIO
    {
        ...
        const string EWS_OUTAGE_ERROR = "Pre-screening unavailable"; 
        ...
        public bool EWSScanningCustomerRegistration(CustomerProfile customer, RCIFCredential credential, ZeoContext context)
        {
            ...
            try
            {
                ...
                //If Prescanning is failed with error message, display the error message we get from the EWS.
                if (!string.IsNullOrWhiteSpace(preScreenResponse?.cpsErrorMessage)) 
                {
                    string errorMessage = preScreenResponse.cpsErrorMessage;

                    //Checking for Outage Error from EWS. 
                    if (errorMessage.Contains(EWS_OUTAGE_ERROR))
                    {
                        throw new CustomerProviderException(CustomerProviderException.CUSTOMER_REGISTRATION_EWS_OUTAGE_ERROR, null, null);
                    }
                    else
                    {
                        throw new CustomerProviderException(CustomerProviderException.CUSTOMER_REGISTRATION_EWS_SCANNING_IS_FAILED, null, null);
                    }
                }
                //In case if the cpsSuccessIndicator is not "S" and if the error message is not there in the flag.
                else
                {
                    throw new Exception();
                }                    
