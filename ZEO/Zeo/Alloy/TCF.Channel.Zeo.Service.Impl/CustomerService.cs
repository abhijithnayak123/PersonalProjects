...
namespace TCF.Channel.Zeo.Service.Impl
{
    public partial class ZeoServiceImpl : ICustomerService
    {
        ...
        public Response IsCardValidForCustomer(string cardnumber, Data.ZeoContext context)
        {
            commonData.ZeoContext commonContext = mapper.Map<commonData.ZeoContext>(context);

            Response response = new Response();
            CustomerService = new CustomerService();
            response.Result = CustomerService.IsCardValidForCustomer(cardnumber, commonContext);
            return response;
        }
