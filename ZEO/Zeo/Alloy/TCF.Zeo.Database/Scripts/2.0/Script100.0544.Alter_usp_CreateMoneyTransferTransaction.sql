...
BEGIN TRY
    ...
    FROM 
       tshoppingCarts sc
       INNER JOIN tShoppingCartTransactions sct ON sc.customersessionid = @customerSessionId AND sc.Status = 1 AND sc.cartid = sct.cartid AND sct.ProductId = 3
       INNER JOIN tTxn_MoneyTransfer mt ON sct.TransactionId = mt.TransactionId
    WHERE 
       mt.ConfirmationNumber = @confirmationNumber AND TransferType = '2' AND mt.[State] = 2 
