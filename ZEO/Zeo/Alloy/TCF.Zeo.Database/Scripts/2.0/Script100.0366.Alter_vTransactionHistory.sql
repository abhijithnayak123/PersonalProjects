...
-- Jira ID:		<AL-8869>
-- ===========================================================================================
--TODO Need to work on location changes once location and terminal is done
...
SELECT   
	...
	CASE tC.ProviderId
		WHEN 200 THEN
		  (
		     CASE sCH.CheckNumber WHEN '' THEN 'Not Available' ELSE ISNULL('[' + tCT.Name  + '] [' + sCH.CheckNumber + ']','Not Available') END	
	 	  )  
		ELSE 'Not Available'
...
FROM   
	...
	LEFT JOIN tChxr_Trx sCH WITH (NOLOCK) ON sCH.ChxrTrxID = tC.CXNId AND tC.ProviderId = 200
	LEFT JOIN tCheckTypes tCT WITH (NOLOCK) ON tCT.CheckTypeId = tC.CheckType
WHERE   
		 ...
		 WHEN 103 THEN 
		    (
			   CASE tF.FundType
				  ...
				  ELSE ISNULL('Card #[' + right(sVA.CardNumber,4) + '] ', 'Not Available')
			   END
			)		 
		 ELSE 'Not Available'
    END AS TransactionDetail,    
...
FROM  
	...
	LEFT JOIN tVisa_Trx sV WITH (NOLOCK) ON sV.VisaTrxID = tF.CXNId AND tF.ProviderId = 103
	LEFT JOIN tVisa_Account sVA WITH (NOLOCK) ON sVA.VisaAccountID = sV.VisaAccountId
	LEFT JOIN tCustomers sCC WITH (NOLOCK) ON tF.AddOnCustomerId = sCC.CustomerID
WHERE   
...
FROM   
	...
	INNER JOIN tLocations tL WITH (NOLOCK) ON tT.LocationId = tL.LocationID
	INNER JOIN tCustomers sC WITH (NOLOCK) ON sC.CustomerID = tCS.CustomerID  
	LEFT JOIN tWUnion_BillPay_Trx sBW WITH (NOLOCK) ON sBW.WUBillPayTrxID= tC.CXNId AND tC.ProviderId = 401 
WHERE   
	tC.DTTerminalCreate > DATEADD(d, -90, GETDATE())
...
SELECT   
	...
	CASE  tC.TransferType 
		...
		WHEN '2' THEN 'from [' + C.FirstName + ' ' + C.MiddleName + ' ' + C.LastName + ']'
		END AS TransactionDetail,		   
	C.FirstName + ' ' + C.LastName AS CustomerName,  
	CASE tC.State   
		WHEN 1 THEN 'Pending'     
...
FROM   
	...
	INNER JOIN tLocations tL WITH (NOLOCK) ON tT.LocationId = tL.LocationID 
	INNER JOIN tCustomers C WITH (NOLOCK) ON C.CustomerID = tCS.CustomerID                                       
	LEFT JOIN tWUnion_Trx sW WITH (NOLOCK) ON tC.CXNId = sW.WUtrxId AND tC.ProviderId = 301 
...
FROM   
	tTxn_Cash tCA  
	INNER JOIN tCustomerSessions tCS ON tCS.CustomerSessionID = tCA.CustomerSessionId  
	...
	INNER JOIN tTerminals tT ON tAS.TerminalId = tT.TerminalID
	INNER JOIN tLocations tL ON tT.LocationId = tL.LocationID 
	INNER JOIN tCustomers sC ON sC.CustomerID = tCS.CustomerID
WHERE   
