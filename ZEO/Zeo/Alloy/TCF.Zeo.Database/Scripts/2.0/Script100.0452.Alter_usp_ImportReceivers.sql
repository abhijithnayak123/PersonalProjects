...
BEGIN TRY
	...
	INNER JOIN #TempReceivers tr ON 
		...
		AND wr.Country = tr.PickupCountry
	WHERE 
		tr.ReceiverIndexNo IS NOT NULL 
	...
	-- Insert the receiver which FirstName and LastName exist in alloy with differnt country
	INSERT INTO tWUnion_Receiver
	(
		FirstName,
		LastName,
	...
	FROM 
		#TempReceivers tr
	LEFT JOIN tWUnion_Receiver wr ON 
		wr.FirstName = tr.FirstName 
		AND wr.LastName = tr.LastName 
		AND wr.PickupCountry != tr.PickupCountry
	WHERE
		 tr.ReceiverIndexNo IS NOT NULL	 
		 AND wr.FirstName IS NOT NULL 
		 AND wr.LastName IS NOT NULL
		 AND wr.CustomerId = tr.CustomerId 
