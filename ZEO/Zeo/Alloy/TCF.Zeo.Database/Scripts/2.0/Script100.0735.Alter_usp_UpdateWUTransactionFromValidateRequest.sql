...
-- Modified Description: Updating the Fraud Consent in the table for validation request.
-- Jira ID:		<AL-8324>
-- ================================================================================
...
(
	...
	,@consumerFraudPromptQuestion VARCHAR(1)
)
AS
BEGIN
	
BEGIN TRY
		---In receipt we need to display transalated delivery service name
		SELECT 
		 	TOP 1 @transalatedDeliveryServiceName = TanslatedDeliveryServiceName 
		FROM 
			tWunion_DeliveryTranslations
		WHERE 
			EnglishName = @deliveryServiceDesc AND LanguageCode = 'es'
		...
		SET 
			...
			,ConsumerFraudPromptQuestion = @consumerFraudPromptQuestion
