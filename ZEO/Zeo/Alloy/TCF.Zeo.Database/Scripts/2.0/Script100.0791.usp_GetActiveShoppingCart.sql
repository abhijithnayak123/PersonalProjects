...
-- Modifications : Adding the TCF Onus tables for Check Transacations.
-- Jira ID:		<>
-- ================================================================================
...
CREATE PROCEDURE [dbo].[usp_GetActiveShoppingCart]
(
	...
	BEGIN TRY
     ...
     BEGIN
        ...
        SELECT 
           ...
           tc.Amount,
	       tc.Fee,
	       ISNULL(tc.ShoppingCartDescription,tc.Description) AS Description,
	       tc.[State] AS State,
	       tc.BaseFee,
	       tc.DiscountApplied,
	       tc.DiscountName,
	       ct.DisplayMessage AS DeclineMessage,	    
	    ...
	    INNER JOIN
	       ...
	       ON sct.TransactionId = tc.TransactionId AND sct.CartId = @cartId AND sct.ProductId = 1 AND sct.CartItemStatus = 0
	    LEFT JOIN
	       tChxr_Trx ct WITH (NOLOCK)
	       ON tc.CXNId = ct.ChxrTrxID 
		LEFT JOIN
	       tTCFOnus_Trx ust WITH (NOLOCK)
	       ON tc.CXNId = ust.TCFOnusTrxID 
	    ...
	    -- Get Money Transfer transaction
         ...
         SELECT 
			 ...
			 CASE WHEN wmt.TranascationType = 2 THEN wmt.SenderName ELSE c.FirstName END AS FirstName,
			 CASE WHEN wmt.TranascationType = 2 THEN '' ELSE c.LastName END AS LastName,
			 CASE WHEN wmt.TranascationType = 2 THEN '' ELSE c.MiddleName  END AS MiddleName,
			 CASE WHEN wmt.TranascationType = 2 THEN '' ELSE c.LastName2 END AS LastName2,			
			 wmt.Charges,
			 sct.ProductId as ProductId			 
        FROM 
		...
		WHERE 
			 ...
			 sct.CartItemStatus = 0
			 AND
			 ( mt.TransactionSubType != 3 OR  mt.TransactionSubType IS NULL )  --- Refund transaction will not be displaying in shopping cart
