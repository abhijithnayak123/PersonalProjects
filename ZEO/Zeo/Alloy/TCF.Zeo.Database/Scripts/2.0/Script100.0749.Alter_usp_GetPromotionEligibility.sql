	...
	BEGIN TRY
	 ...
	 WHERE  @currentDate BETWEEN StartDate AND EndDate AND Status = 1 
			...
			(
				Name = @promoCode
				OR 
				@promoCode = '' AND IsPromotionHidden = 0      -- AND IsSystemApplied = 1  -- In case of manual promotion, we will get promo code value in field - '@promoCode' or else we will check all the Promos.
			)														 
			AND ProductId = @productId AND (ProviderId = @providerId OR ProviderId IS NULL)
	 ...
	 SELECT
		...
		pv.PromoProvisionId AS ProvisionId 
		FROM 
	  ...
	  WHERE
		...
		GROUP BY p.PromotionId, p.Name, p.IsOverridable, p.Description, p.Priority, pv.Groups, pv.PromoProvisionId              -- Take max discount value, if we have two provisions for same promotions
