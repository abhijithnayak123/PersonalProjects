...
BEGIN
    BEGIN TRY
			...
			DECLARE @cartId                 BIGINT   
					...
					FROM  tShoppingCarts 
					WHERE dbo.tShoppingCarts.CustomerSessionId = @customerSessionId AND State = 3
					...
					WHERE tSC.CartID=@cartId AND tTC.State=4
					...
					FROM   tTxn_MoneyTransfer tTC
							...
							JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND TransferType = 1 AND TransactionSubType is NULL OR TransactionSubType = ''

					SELECT @MoneyTransferReceive = ISNULL(SUM(Amount),0.00),
							@RMCount = COUNT(1) FROM tTxn_MoneyTransfer tTC
							JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 3
							JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND TransferType = 2 AND tTC.State=4 

					SELECT @MoneyTransferCancelled = ISNULL(SUM(Amount),0.00) + ISNULL(SUM(Fee),0.00) 
					FROM   tTxn_MoneyTransfer tTC
							JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 3
							JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND TransferType = 1 AND TransactionSubType = 1

					SELECT @MoneyTransferModified = ISNULL(SUM(Amount),0.00) + ISNULL(SUM(Fee),0.00) 
					FROM   tTxn_MoneyTransfer tTC
							JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 3
							JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND TransferType = 1 AND TransactionSubType=2

					SELECT @MoneyTransferRefund = ISNULL(SUM(Amount),0.00) + ISNULL(SUM(Fee),0.00) 
					FROM   tTxn_MoneyTransfer tTC
							JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 3
							JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND TransferType = 1 AND TransactionSubType=3
					...
					FROM tTxn_Funds tTC
						...
						JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND FundType = 1

					SELECT @GPRWithDraw = ISNULL(SUM(Amount),0.00)
					FROM tTxn_Funds tTC
						JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 6
						JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND FundType = 0

					SELECT @GPRActivate = ISNULL(SUM(Amount),0.00) + ISNULL(SUM(Fee),0.00) 
					FROM tTxn_Funds tTC
						JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 6
						JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND tTC.State=4 AND FundType = 2

					SELECT @IsAddOn = COUNT(*) 
					FROM tTxn_Funds tTC
						JOIN tShoppingCartTransactions tSCT ON tSCT.TransactionId = tTC.TransactionID AND tSCT.ProductId = 6
						JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND FundType = 3 AND tTC.State=4
					...
					FROM tTxn_Cash tTC
						...
						JOIN tShoppingCarts tSC ON tSC.CartID = tSCT.CartId
					WHERE tSC.CartID = @cartId AND CashType = 1 AND tTC.State=4
					...
					WHERE  tCS.CustomerSessionID = @customerSessionId and tVA.Activated=1
						...
						SET @FundsGeneratingTotal = @CheckTotal+ @GPRWithDraw + @MoneyTransferReceive + @MoneyTransferCancelled 
