...
-- Modified Description: Updating the stored Proc with the Fraud Limit value.
-- Jira ID:		<AL-8998>
-- ================================================================================
...
BEGIN
 ...
 BEGIN TRY
		...
		SELECT
			...
			[Table].[Column].value('FraudLimit[1]', 'VARCHAR(20)') as 'FraudLimit',
		...
		INSERT INTO 
			tWUnion_CountryCurrencies (CountryName,CountryCode,CountryNumCode,CurrencyCode,FraudLimit,CurrencyNumCode,CurrencyName,DTServerCreate) 
			(SELECT ts.CountryName,ts.CountryCode,TS.CountryNumCode,ts.CurrencyCode,ts.FraudLimit,ts.CurrencyNumCode,ts.CurrencyName,DTServerCreate
