...
DROP PROC usp_GetMoneyTransferReceiptData
GO
...
CASE	WHEN tC.LastName2 IS NULL THEN CONCAT(tC.FirstName, ISNULL(tC.MiddleName, ''), ISNULL(tC.LastName, ''))
		ELSE CONCAT(tC.FirstName, ISNULL(tC.LastName, ''), ISNULL(tC.LastName2, ''))
		END								AS SenderName,
CASE	WHEN tC.LastName2 IS NULL THEN CONCAT(tC.FirstName, ISNULL(tC.MiddleName, ''), ISNULL(tC.LastName, ''))
		ELSE CONCAT(tC.FirstName, ISNULL(tC.LastName, ''), ISNULL(tC.LastName2, ''))
		END								AS CustomerName,
CONCAT(tC.Address1, ', ', ISNULL(tC.Address2, '')) 
										AS SenderAddress,
...
CONCAT(ISNULL(tWT.RecieverFirstName, ''), ISNULL(tWT.RecieverLastName, ''), ISNULL(tWT.RecieverSecondLastName, ''))
										AS ReceiverName,
...
tWT.municipal_tax + tWT.state_tax + tWT.county_tax 
										AS TransferTaxes,
...
FROM tTxn_MoneyTransfer tTM
INNER JOIN tWUnion_Trx tWT				ON tTM.CXNId = tWT.WUTrxId
LEFT  JOIN tWUnion_Receiver tWR			ON tWR.WUReceiverID = tWT.WUReceiverId
INNER JOIN tCustomerSessions tCS		ON tTM.CustomerSessionId = tCS.CustomerSessionID
...
INNER JOIN tWUnion_Countries tWC		ON tWC.ISOCountryCode = tWT.DestinationCountryCode
INNER JOIN tWunion_CountryTranslation t	ON t.ISOCountryCode = tWT.DestinationCountryCode AND t.LanguageCode = 'es'
INNER JOIN tChannelPartnerProductProcessorsMapping tPPM ON tCP.ChannelPartnerPK=tPPM.ChannelPartnerID
INNER JOIN tProductProcessorsMapping tPM    ON tPM.ProductProcessorsMappingPK=tPPM.ProductProcessorId
