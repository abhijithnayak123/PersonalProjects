...
namespace TCF.Zeo.Biz.MoneyTransfer.Contract
{
    public interface IMoneyTransferEngine
    {
		...
		decimal GetFraudLimit(string countryCode, commonData.ZeoContext context);

        decimal GetDestinationAmount(FeeRequest feeRequest, commonData.ZeoContext context);

        WUCountry GetBlockedUnBlockedCountries(commonData.ZeoContext context);

        bool SaveBlockedCountries(List<string> blockedCountries, commonData.ZeoContext context);
    }
}
