    ...
    public partial class ZeoCoreImpl : IFeeService
    {
            ...
            try
            {
                ...
                using (IDataReader datareader = DataConnectionHelper.GetConnectionManager().ExecuteReader(coreChannelPartnerFeeAdjProcedure))
                {
                    ...
                    while (datareader.Read())
                    {
                        ...
                        feeAdjustment.Priority = datareader.GetInt32("Priority");
                        feeAdjustment.IsOverridable = datareader.GetBooleanOrDefault("IsOverridable");
        ...
        public bool ValidatePromoCode(Helper.TransactionType transactionType, long channelPartnerId, string promotionCode, ZeoContext context)
        {
            try
            {
                ...
                DateTime today = DateTime.Today;
                coreChannelPartnerFeeAdjProcedure.WithParameters(InputParameter.Named("productId").WithValue((int)transactionType));
                coreChannelPartnerFeeAdjProcedure.WithParameters(InputParameter.Named("date").WithValue(today));
            ...
            try
            {
                ...
                using (IDataReader datareader = DataConnectionHelper.GetConnectionManager().ExecuteReader(coreTrxProcedure))
                {
                    while (datareader.Read())
                    {
                        ...
                        feeAdjustment.PromotionId = datareader.GetInt64OrDefault("FeeAdjustmentId");
        ...
        public int GetCommittedTrxCountByPromoCode(string promoCode, TransactionType transactionType, long transactionId, bool systemApplied, DateTime dTStart, ZeoContext context)
        {
            try
            {
                int count = 0;

                string usp_Name = "usp_GetCommittedCheckTrxCountByPromoCode";

                if (transactionType == TransactionType.MoneyOrder)
                    usp_Name = "usp_GetCommittedMOTrxCountByPromoCode";
                ...
                using (IDataReader dr = DataConnectionHelper.GetConnectionManager().ExecuteReader(coreTrxProcedure))
                {
                    while (dr.Read())
                    {
                        count = dr.GetInt32OrDefault("CommittedTrxCount");
                    }
                }
        ...
        #region New Promotion Engine code changes

        public List<TransactionFee> GetPromotionEligibility(decimal baseFee, decimal amount, int checkTypeId,
                long transactionId, TransactionType transactionType, ZeoContext context)
        {
            try
            {
                List<TransactionFee> fees = new List<TransactionFee>();

                StoredProcedure coreTrxProcedure = new StoredProcedure("usp_GetPromotionEligibility");
                coreTrxProcedure.WithParameters(InputParameter.Named("PromoCode").WithValue(context.PromotionCode));
                coreTrxProcedure.WithParameters(InputParameter.Named("CustomerSessionId").WithValue(context.CustomerSessionId));
                coreTrxProcedure.WithParameters(InputParameter.Named("BaseFee").WithValue(baseFee));
                coreTrxProcedure.WithParameters(InputParameter.Named("CheckAmount").WithValue(amount));
                coreTrxProcedure.WithParameters(InputParameter.Named("locationStateCode").WithValue(context.LocationID));
                coreTrxProcedure.WithParameters(InputParameter.Named("CheckTypeId").WithValue(checkTypeId));
                coreTrxProcedure.WithParameters(InputParameter.Named("TransactionId").WithValue(transactionId));
                coreTrxProcedure.WithParameters(InputParameter.Named("CurrentDate").WithValue(DateTime.Today));
                coreTrxProcedure.WithParameters(InputParameter.Named("ProductId").WithValue((int)transactionType));
                coreTrxProcedure.WithParameters(InputParameter.Named("CustomerId").WithValue(context.CustomerId));
                coreTrxProcedure.WithParameters(InputParameter.Named("ProviderId").WithValue(context.ProviderId));
                ...
                using (IDataReader dr = DataConnectionHelper.GetConnectionManager().ExecuteReader(coreTrxProcedure))
                {
                    while (dr.Read())
                    {
                        ...
                        {
                            PromotionCode = dr.GetStringOrDefault("Name"),
                            PromotionDescription = dr.GetStringOrDefault("Description"),
                            IsOverridable = dr.GetBooleanOrDefault("IsOverridable"),
                            DiscountApplied = dr.GetDecimalOrDefault("DiscountValue"),
                            PromotionId = dr.GetInt64OrDefault("PromotionId"),
                            Priority = dr.GetInt32OrDefault("Priority"),
                            NetFee = baseFee + dr.GetDecimalOrDefault("DiscountValue"),
                            IsGroupPromo = dr.GetBooleanOrDefault("IsGroupPromo"),
                            BaseFee = baseFee,
                            ProvisionId = dr.GetInt64OrDefault("ProvisionId")
                            
                        });
                    }
                }

                List<TransactionFee> listObjects = (from obj in fees.OrderByDescending(x => x.DiscountApplied)
                                             select obj).GroupBy(n => new { n.PromotionCode, n.PromotionId})
                                           .Select(g => g.FirstOrDefault())
                                           .ToList<TransactionFee>();
