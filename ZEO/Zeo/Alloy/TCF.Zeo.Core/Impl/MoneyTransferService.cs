...
namespace TCF.Zeo.Core.Impl
{
    public partial class ZeoCoreImpl : IMoneyTransferService
    {
        ...
        public WUCountry GetBlockedUnBlockedCountries(ZeoContext context)
        {
            try
            {
                StoredProcedure blockedCountriesProcedure = new StoredProcedure("usp_GetBlockedUnBlockedCountries");

                WUCountry countries = new WUCountry();

                using (IDataReader reader = DataHelper.GetConnectionManager().ExecuteReader(blockedCountriesProcedure))
                {
                    countries.BlockedCountries = new List<Country>();

                    while (reader.Read())
                    {                        
                        countries.BlockedCountries.Add(new Country { CountryName = reader.GetStringOrDefault("CountryName"), ISOCountryCode = reader.GetStringOrDefault("ISOCountryCode") });
                    }

                    reader.NextResult();

                    countries.UnblockedCountries = new List<Country>();

                    while (reader.Read())
                    {                        
                        countries.UnblockedCountries.Add(new Country { CountryName = reader.GetStringOrDefault("CountryName"), ISOCountryCode = reader.GetStringOrDefault("ISOCountryCode") });
                    }
                }
        ...
        public bool SaveBlockedCountries(List<string> blockedCountries, ZeoContext context)
        {
            try
            {
                StoredProcedure blockedCountriesProcedure = new StoredProcedure("usp_InsertBlockedCountries");

                StringWriter blockedCountriesWriter = new StringWriter();

                DataSet blockedCountriesDataset = GetBlockedCountriesDataset(blockedCountries, context);
            ...
            {
                throw new MoneyTransferException(MoneyTransferException.INSERT_BLOCKED_COUNTRIES_FAILED, ex);
            }
        }

        private DataSet GetBlockedCountriesDataset(List<string> blockedCountries, ZeoContext context)
        {
            DataSet dataSet = new DataSet("BlockedCountries");
            DataTable table = new DataTable("BlockedCountry");
            table.Columns.Add("ISOCountryCode", typeof(string));
            table.Columns.Add("DTServerDate", typeof(DateTime));
            table.Columns.Add("DTTerminalDate", typeof(DateTime));

            foreach (string blockedCountry in blockedCountries)
            {
                DataRow dr = table.NewRow();
                dr["ISOCountryCode"] = blockedCountry;
