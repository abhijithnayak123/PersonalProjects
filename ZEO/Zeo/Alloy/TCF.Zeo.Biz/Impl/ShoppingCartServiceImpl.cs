...
namespace TCF.Zeo.Biz.Impl
{
    public class ShoppingCartServiceImpl : IShoppingCartService
    {
        ...
        public bool ParkShoppingCartTransaction(long customerSessionId, long transactionId, long productId, commonData.ZeoContext context)
        {
            try
            {
                return ShoppingCartService.ParkShoppingCartTransaction(customerSessionId, transactionId, productId, context.TimeZone, context);
            }
            catch (Exception ex)
            {
                if (ExceptionHelper.IsExceptionHandled(ex)) throw;
        ...
        private void CommitChecks(List<CoreData.Data.ShoppingCartTransaction> cartTransactions, commonData.ZeoContext context)
        {
            ...
            {
                context.ProviderId = (int)check.ProviderId;
                if (CPService == null)
