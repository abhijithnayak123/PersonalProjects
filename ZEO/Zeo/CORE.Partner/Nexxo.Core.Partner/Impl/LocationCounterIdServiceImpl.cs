...
namespace MGI.Core.Partner.Impl
{
	public class LocationCounterIdServiceImpl : ILocationCounterIdService
	{
		...
		public string Get(Guid locationID, int providerId)
		{
			...
			{
				throw new PartnerLocationException(PartnerLocationException.LOCATION_COUNTERID_GET_FAILED, ex);
			}
		...
		public bool Update(LocationCounterId locationCounterId)
		{
			...
			catch (Exception ex)
			{
				...
				throw new PartnerLocationException(PartnerLocationException.LOCATION_COUNTERID_STATUS_UPDATE_FAILED, ex);
			}
		}
	}
}
